...
on *:load:{ 
  ...
  echo -a  4Team Nexgen's Secure Query v2.2 | echo -a  4Please type: /secure.query in channel to begin dialog.
}
...
on *:dialog:*:sclick:*:{ 
  if (Query request $dname == $dialog($dname).title) { 
    ...
    if ($did == 3) {
      ...
      if ($_vr(querycon,notifyaccess) == on) { if ($server != $null) { .msg $did($dname,1) [automated msg] You have free access to message me. } }
    }   
    if ($did == 27) { .ignore -pu180 $did($dname,30) | .msg $did($dname,1) [automated msg] $_vr(querycon,decline) |  dialog -x $did($dname,1) }
    if ($did == 28) {  
      .ignore -r $address($did($dname,1),2) | query $did($dname,1) | .timer -m 1 1 echo $did($dname,1) $stimestamp < $+ $did($dname,1) $+ > $did(29,$did(29)) |  _vrem querycon lastmsg 
      if ($server != $null) { .msg $did($dname,1) [automated msg] Query request accepted. }
...
alias -l sw {
  if ($exists(securequery\logs.ini) == $false) { mkdir securequery | write -c securequery\logs.ini }
  if ($file(securequery\logs.ini).size >= 40000) { write -c securequery\logs.ini }
...
on ^*:open:?: {
  ...
  if ($_vr(querycon,switch) == on) {
    ...
    echo -a  $+ $colour(notice) $+ Query with $nick on $network
    echo -a  $+ $colour(notice) $+ Waiting for acknowledgement...
    if ($_vr(querycon,log) == on) { 
      if ($read(securequery\names.txt, w, $nick) == $nick) { write -ds $+ $nick securequery\names.txt }
      sw $nick date $asctime(h:nn:sstt m/d) | sw $nick msg $strip($1-,burc) | write securequery/names.txt $nick | nick.list
    }
...
raw 315:*: {
  .timer 1 10 .disable #sqialud | halt
} 
#sqialud end
...
alias stimestamp {
  if ($nexgen == $true) { 
    if ($_vr(color,menick_switch) == off) || ($group(#colortheme) == off) { return }
    elseif ($_vr(general,timestamp) == on) { return $timestamp } 
  }
  elseif ($nexgen $= $true) { return $timestamp }
}
