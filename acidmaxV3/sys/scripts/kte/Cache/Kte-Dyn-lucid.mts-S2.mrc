...
if ($nick == $me) { %:echo 15–› Now talking in : %::chan $+  %:comments | haltdef }
else { %:echo 15–› %::nick has joined %::chan $+  %:comments | haltdef }
...
%:echo 15–› %::nick has left : %::chan %::parentext $+  %:comments | haltdef
...
if (* iswm %::text) { set -nu1 %::parentext ( $+ %::text $+ ) }
  while (%i) {
    ...
    if (%tgt = 3) || (%tgt = 2) { %st = 1 }
if (!%tgt) || (%tgt = 3) || (%tgt = 1) { %:echo = echo $color(quit) -ti2 %::chan | %:echo 15–› %::nick Quit %::parentext $+  %:comments }
    dec %i
  }
  unset %::chan
if ($query($nick)) { %:echo = echo $color(quit) -ti2 $nick | %:echo 15–› %::nick Quit %::parentext $+  %:comments }
if (%st) { %:echo = echo $color(quit) -sti2 | %:echo 15–› %::nick Quit %::parentext $+  %:comments }
  haltdef
...
if ($nick == $me) { %:echo = echo $color(nick) -sti2 | %:echo 15–› %::me is now known as %::newnick $+  %:comments }
  while (%i) {
    set -u1 %::chan $comchan($newnick, %i)
    %tgt = $gettok($hget(Kte_Events, %::chan), 7, 44)
if (%tgt != 2) || ($nick == $me) { %:echo = echo $color(nick) -ti2 %::chan | %:echo 15–› %::nick is now known as : %::newnick $+  %:comments }
    dec %i
  }
if ($query($newnick)) { unset %::chan | %:echo = echo $color(nick) -ti2 $newnick | %:echo 15–› %::nick is now known as : %::newnick $+  %:comments }
  haltdef
...
%:echo 15–› %::me sets mode : %::modes $+  %:comments | haltdef
  unset %::modes
...
set -nu1 %::chan $chan
  ...
  set -nu1 %::modes $1-
%:echo 15–› %::nick sets mode : %::modes $+  %:comments | haltdef
  unset %::modes
unset %:echo %::nick %::address %::target %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick
}
...
set -nu1 %::chan $chan
if (* iswm %::text) { set -nu1 %::parentext ( $+ %::text $+ ) }
%:echo 15–› %::nick changes topic to : %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
on ^&*:KICK:#:{
  if ($chan !ischan) { return }
...
if (* iswm %::text) { set -nu1 %::parentext ( $+ %::text $+ ) }
  if ($knick == $me) {
%:echo 15–› %::me was kicked by : %::nick %::parentext $+  %:comments
    if (%tgt == 2) { set -u1 %:echo echo $color(kick) -ti2 $chan }
    else { set -u1 %:echo echo $color(kick) -seti2 }
%:echo 15–› %::me was kicked by : %::nick %::parentext $+  %:comments
    _left $chan
  }
else { %:echo 15–› %::knick was kicked by : %::nick %::parentext $+  %:comments }
  haltdef
...
%:echo 15–› %::nick invites you to join : %::chan $+  %:comments | haltdef
...
set -nu1 %::address $address
if (* iswm %::text) { set -nu1 %::parentext ( $+ %::text $+ ) }
%:echo 15–›  $+ %::nick $+  is now online %:comments | haltdef
unset %:echo %::nick %::address %::parentext %::me %::server %::port %::pre %::c? %::text
}
on ^&*:UNOTIFY:{
  set -u1 %:echo echo $color(notify) -ti2 $+ $iif(($_isactive) && (@* !iswm $active), a, s)
...
set -nu1 %::address $address
if (* iswm %::text) { set -nu1 %::parentext ( $+ %::text $+ ) }
%:echo 15–›  $+ %::nick $+  is now offline %:comments | haltdef
unset %:echo %::nick %::address %::parentext %::me %::server %::port %::pre %::c? %::text
}
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext ( $+ %::text $+ ) }
%:echo 15–› Wallops –› %::nick %::text $+  %:comments | haltdef
...
%:echo 15–› Resolved %::address to %::raddress $+  %:comments | haltdef
      inc %i
    }
  }
  else {
    set -nu1 %::nick $nick
    set -nu1 %::address $address
%:echo 15–› Unable to resolve %::address $+  %:comments | haltdef
  }
...
%:echo 12( $+ %::cmode $+ %::me $+ 12) %::text $+  %:comments
  }
  else {
    set -u1 %::cnick $cnick($me).color
%:echo 12( $+ %::nick $+ 12) %::text $+  %:comments
  }
...
else { %:echo 12( $+ %::nick $+ 12) %::text $+  %:comments }
  }
  else {
...
if ($1 ischan) { set -u1 %::chan $1 | %:echo - $+ %::target $+ - %::text $+  %:comments }
else { %:echo - $+ %::target $+ - %::text $+  %:comments }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext ( $+ %::text $+ ) }
%:echo 12( $+ %::cmode $+ %::nick $+ 12) %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
on &^*:ACTION:*:#:{
  if ($chan !ischan) { return }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext ( $+ %::text $+ ) }
%:echo › %::cmode $+ %::nick %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
on &^*:NOTICE:*:#:{
  if ($chan !ischan) { return }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext ( $+ %::text $+ ) }
%:echo - $+ %::nick $+ @ $+ %::chan $+ - %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
...
%:echo › %::nick %::text $+  %:comments | haltdef
    ...
    set -nu1 %::text $strip($1-, o)
if (* iswm %::text) { set -nu1 %::parentext ( $+ %::text $+ ) }
%:echo 12( $+ %::nick $+ 12) %::text $+  %:comments | haltdef
  }
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::text
}
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext ( $+ %::text $+ ) }
if (%ch) { %:echo ( $+ %::cmode $+ %::nick $+ : $+ %::chan %::ctcp $+ ) %::text $+  %:comments }
else { %:echo ( $+ %::nick %::ctcp $+ ) %::text $+  %:comments }
...
if ($1 ischan) { set -u1 %::chan $1 | %:echo –› ( $+ %::target $+ ) %::ctcp %::text $+  %:comments }
else { set -u1 %::nick $1 | %:echo –› ( $+ %::target $+ ) %::ctcp %::text $+  %:comments }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext ( $+ %::text $+ ) }
%:echo [ $+ %::nick %::ctcp $+ ] %::text $+  %:comments | haltdef
...
set -nu1 %::pre 15–›
if (* iswm %::text) { set -nu1 %::parentext ( $+ %::text $+ ) }
%:echo –› [ $+ %::target $+ ] %::ctcp %::text $+  %:comments
...
set -nu1 %::me $me | set -nu1 %::server $server | set -nu1 %::port $port
set -nu1 %::pre 15–›
%:echo 15–› Looking up %::address $+  %:comments
...
alias -l kte::Echo %:echo 15–› %::text $+  %:comments
alias -l kte::EchoTarget %:echo 15–› %::text $+  %:comments
alias -l kte::Error %:echo 15–› Error › %::text $+  %:comments
alias -l kte::Raw.303 %:echo 15–› %::text are online %:comments
alias -l kte::Raw.332 %:echo 15–› Topic is %::text $+  %:comments
alias -l kte::RAW.333 %:echo 15–› Set by %::nick on %::text $+  %:comments
alias -l kte::RAW.330 set -u4 %::clwhois.auth $iif($gettok(%::text,4,32) != as,$ifmatch,$gettok(%::text,1,32))
alias -l kte::Raw.251 %:echo There are %::users user $+ $iif((%::users != 1), s) online and %::text invisible on %::value servers
alias -l kte::Raw.301 %:echo 15–› %::nick is away: %::text $+  %:comments
alias -l kte::Raw.302 %:echo 15–› Userhost: %::nick $+ = $+ %::value $+ %::address $+  %:comments
