...
; Kte-Dyn 1.5 FF4BFBB0 53326CE1
alias theme.text {
  if (!$var(%:echo)) { return $false }
set -nu1 %::me $me | set -nu1 %::server $server | set -nu1 %::port $port
set -nu1 %::pre 10~
set -nu1 %::c1 11 | set -nu1 %::c2 12 | set -nu1 %::c3 10 | set -nu1 %::c4 11
...
if ($nick == $me) { %:echo 10~11 [12 $+ %::chan $+ 11] %:comments | haltdef }
...
%:echo 10~11 part: %::nick 10[12 $+ %::address $+ 10] (12 $+ %::text $+ 10) $+  %:comments | haltdef
...
if ($nick == $me) { %:echo = echo $color(nick) -sti2 | %:echo 10~11 nick: %::newnick $+  %:comments }
  while (%i) {
    set -u1 %::chan $comchan($newnick, %i)
    %tgt = $gettok($hget(Kte_Events, %::chan), 7, 44)
if (%tgt != 2) || ($nick == $me) { %:echo = echo $color(nick) -ti2 %::chan | %:echo 10~11 nick: %::nick 10[12 $+ %::address $+ 10]11 is now %::newnick $+  %:comments }
    dec %i
  }
if ($query($newnick)) { unset %::chan | %:echo = echo $color(nick) -ti2 $newnick | %:echo 10~11 nick: %::nick 10[12 $+ %::address $+ 10]11 is now %::newnick $+  %:comments }
  haltdef
...
%:echo 10~11 usermode: %::modes $+  %:comments | haltdef
  unset %::modes
...
set -nu1 %::chan $chan
  ...
  set -nu1 %::modes $1-
%:echo 10~11 mode: %::nick 10[12 $+ %::address $+ 10] 11set: %::modes $+  %:comments | haltdef
  unset %::modes
unset %:echo %::nick %::address %::target %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick
}
...
set -nu1 %::chan $chan
if (* iswm %::text) { set -nu1 %::parentext 12[11 $+ %::text $+ 12] }
%:echo 10~11 %::nick sets topic: 10(12 $+ %::text $+ 10) $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
on ^&*:KICK:#:{
  if ($chan !ischan) { return }
...
if (* iswm %::text) { set -nu1 %::parentext 12[11 $+ %::text $+ 12] }
  if ($knick == $me) {
%:echo 10~11 You were kicked from %::chan by %::nick 10(12 $+ %::text $+ 10) $+  %:comments
    if (%tgt == 2) { set -u1 %:echo echo $color(kick) -ti2 $chan }
    else { set -u1 %:echo echo $color(kick) -seti2 }
%:echo 10~11 You were kicked from %::chan by %::nick 10(12 $+ %::text $+ 10) $+  %:comments
    _left $chan
  }
else { %:echo 10~11 kick: %::nick 10[12 $+ %::address $+ 10]11 kicked %::knick 10(12 $+ %::text $+ 10) $+  %:comments }
  haltdef
...
%:echo 10~11 invite: %::nick 10(12 $+ %::address $+ 10)11 invites you to join %::chan $+  %:comments | haltdef
...
set -nu1 %::nick $nick
if (* iswm %::text) { set -nu1 %::parentext 12[11 $+ %::text $+ 12] }
%:echo 10~11 error: %::text $+  %:comments | haltdef
...
set -nu1 %::address $address
if (* iswm %::text) { set -nu1 %::parentext 12[11 $+ %::text $+ 12] }
%:echo 10~11 nick: %::nick 10(12 $+ %::address $+ 10) is on irc %:comments | haltdef
unset %:echo %::nick %::address %::parentext %::me %::server %::port %::pre %::c? %::text
}
on ^&*:UNOTIFY:{
  set -u1 %:echo echo $color(notify) -ti2 $+ $iif(($_isactive) && (@* !iswm $active), a, s)
...
set -nu1 %::address $address
if (* iswm %::text) { set -nu1 %::parentext 12[11 $+ %::text $+ 12] }
%:echo 10~11 nick: %::nick 10(12 $+ %::address $+ 10) is off irc %:comments | haltdef
unset %:echo %::nick %::address %::parentext %::me %::server %::port %::pre %::c? %::text
}
...
alias -l kte::Error %:echo 10~11error: %::text $+  %:comments
alias -l kte::Raw.303 %:echo 10~ %::text are online %:comments
alias -l kte::RAW.375 %:echo 10~11MOTD - %::server $+  %:comments
alias -l kte::RAW.255 %:echo 10~11I have %::users clients and %::value servers %:comments
alias -l kte::RAW.471 %:echo 10~11error:  $+ %::chan $+  is full %:comments
alias -l kte::RAW.391 %:echo 10~11server time: %::text $+  %:comments
alias -l kte::Raw.332 %:echo 10~11 topic: 10(12 $+ %::text $+ 10)11 $+  %:comments
alias -l kte::RAW.372 %:echo 11 $+ %::text $+  %:comments
alias -l kte::RAW.252 %:echo $+(%::pre,,%::c1,%::value) $iif(%::value == 1,operator,operators) online
alias -l kte::Raw.408 %:echo 10~11error:  $+ %::chan $+  you cannot use colors on this channel %:comments
...
alias -l kte::RAW.265 %:echo 10~11Current local users: %::users max: %::value $+  %:comments
alias -l kte::RAW.253 %:echo 10~11 $+ %::value unknown connections %:comments
alias -l kte::RAW.001 %:echo 11Welcome %::text $+  %:comments
alias -l kte::RAW.482 %:echo 10~11error:  $+ %::chan $+  you aren't an op %:comments
alias -l kte::RAW.401 %:echo 10~11error:  $+ %::nick $+  no such nickname %:comments
alias -l kte::RAW.266 %:echo 10~11Current global users: %::users max: %::value $+  %:comments
alias -l kte::RAW.250 %:echo 10~11 $+ %::value total connections %:comments
alias -l kte::RAW.002 %:echo 11Your Host Is %::server $+ , running %::value $+  %:comments
alias -l kte::RAW.474 %:echo 10~11error:  $+ %::chan $+  banned from channel %:comments
alias -l kte::RAW.003 %:echo 11This server was created on %::value $+  %:comments
alias -l kte::Raw.251 %:echo 10~11There are %::users users online, %::text invisible, on %::value servers %:comments
alias -l kte::RAW.475 %:echo 10~11error:  $+ %::chan $+  key required %:comments
alias -l kte::RAW.324 %:echo 10~11 modes: 10[12 $+ %::modes $+ 10]11 %:comments
alias -l kte::RAW.403 %:echo 10~11error:  $+ %::chan $+  no such channel %:comments
alias -l kte::RAW.376 %:echo 10~11end of MOTD %:comments
alias -l kte::RAW.221 %:echo 10~11 usermode: %::modes $+  %:comments
alias -l kte::Raw.301 %:echo 10~ %::nick is away: %::text $+  %:comments
alias -l kte::RAW.432 %:echo 10~11error:  $+ %::nick $+  invalid nickname %:comments
alias -l kte::RAW.404 %:echo 10~11error:  $+ %::chan $+  unable to send message %:comments
alias -l kte::RAW.353 %:echo 10~11names 10(12 $+ %::chan $+ 10):11 %::text $+  %:comments
alias -l kte::RAW.341 %:echo 10~11 $+ %::nick invited to %::chan $+  %:comments
alias -l kte::RAW.005 %:echo %::text $+  %:comments
alias -l kte::RAW.473 %:echo 10~11error:  $+ %::chan $+  invite only %:comments
alias -l kte::Raw.302 %:echo 10~11 userhost %::nick $+ : %::address $+  %:comments
alias -l kte::RAW.433 %:echo 10~11error:  $+ %::nick $+  is in use %:comments
alias -l kte::RAW.421 %:echo 10~11error:  $+ %::value $+  invalid command %:comments
alias -l kte::RAW.366 %:echo 10~11end of /names %::chan $+  %:comments
