...
; Kte-Dyn 1.5 FF4BFBB0 88401FAC
alias theme.text {
  if (!$var(%:echo)) { return $false }
set -nu1 %::me $me | set -nu1 %::server $server | set -nu1 %::port $port
set -nu1 %::pre     11»11» 12
set -nu1 %::c1 12 | set -nu1 %::c2 11 | set -nu1 %::c3 11 | set -nu1 %::c4 12
...
if ($nick == $me) { %:echo     11»11» 1212 [11 $+ %::chan $+ 12] %:comments | haltdef }
...
if ($nick($chan, $nick).pnick != $nick) { set -nu1 %::cmode $left($ifmatch, 1) } | set -nu1 %::cnick $nick($chan, $nick).color
set -nu1 %::chan $chan
if (* iswm %::text) { set -nu1 %::parentext 11[12 $+ %::text $+ 11] }
%:echo     11»11» 1212 part: %::nick 11[11 $+ %::address $+ 11] (11 $+ %::text $+ 11) $+  %:comments | haltdef
...
if ($nick == $me) { %:echo = echo $color(nick) -sti2 | %:echo     11»11» 1212 nick: %::newnick $+  %:comments }
  while (%i) {
    set -u1 %::chan $comchan($newnick, %i)
    %tgt = $gettok($hget(Kte_Events, %::chan), 7, 44)
if (%tgt != 2) || ($nick == $me) { %:echo = echo $color(nick) -ti2 %::chan | %:echo     11»11» 1212 nick: %::nick 11[11 $+ %::address $+ 11]12 is now %::newnick $+  %:comments }
    dec %i
  }
if ($query($newnick)) { unset %::chan | %:echo = echo $color(nick) -ti2 $newnick | %:echo     11»11» 1212 nick: %::nick 11[11 $+ %::address $+ 11]12 is now %::newnick $+  %:comments }
  haltdef
...
%:echo     11»11» 1212 usermode: %::modes $+  %:comments | haltdef
  unset %::modes
...
set -nu1 %::chan $chan
  ...
  set -nu1 %::modes $1-
%:echo     11»11» 1212 mode: %::nick 11[11 $+ %::address $+ 11] 12set: %::modes $+  %:comments | haltdef
  unset %::modes
unset %:echo %::nick %::address %::target %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick
}
...
set -nu1 %::chan $chan
if (* iswm %::text) { set -nu1 %::parentext 11[12 $+ %::text $+ 11] }
%:echo     11»11» 1212 %::nick sets topic: 11(11 $+ %::text $+ 11) $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
on ^&*:KICK:#:{
  if ($chan !ischan) { return }
...
if (* iswm %::text) { set -nu1 %::parentext 11[12 $+ %::text $+ 11] }
  if ($knick == $me) {
%:echo     11»11» 1212 You were kicked from %::chan by %::nick 11(11 $+ %::text $+ 11) $+  %:comments
    if (%tgt == 2) { set -u1 %:echo echo $color(kick) -ti2 $chan }
    else { set -u1 %:echo echo $color(kick) -seti2 }
%:echo     11»11» 1212 You were kicked from %::chan by %::nick 11(11 $+ %::text $+ 11) $+  %:comments
    _left $chan
  }
else { %:echo     11»11» 1212 kick: %::nick 11[11 $+ %::address $+ 11]12 kicked %::knick 11(11 $+ %::text $+ 11) $+  %:comments }
  haltdef
...
%:echo     11»11» 1212 invite: %::nick 11(11 $+ %::address $+ 11)12 invites you to join %::chan $+  %:comments | haltdef
...
set -nu1 %::nick $nick
if (* iswm %::text) { set -nu1 %::parentext 11[12 $+ %::text $+ 11] }
%:echo     11»11» 1212 error: %::text $+  %:comments | haltdef
...
set -nu1 %::address $address
if (* iswm %::text) { set -nu1 %::parentext 11[12 $+ %::text $+ 11] }
%:echo     11»11» 1212 nick: %::nick 11(11 $+ %::address $+ 11) is on irc %:comments | haltdef
unset %:echo %::nick %::address %::parentext %::me %::server %::port %::pre %::c? %::text
}
on ^&*:UNOTIFY:{
  set -u1 %:echo echo $color(notify) -ti2 $+ $iif(($_isactive) && (@* !iswm $active), a, s)
...
set -nu1 %::address $address
if (* iswm %::text) { set -nu1 %::parentext 11[12 $+ %::text $+ 11] }
%:echo     11»11» 1212 nick: %::nick 11(11 $+ %::address $+ 11) is off irc %:comments | haltdef
unset %:echo %::nick %::address %::parentext %::me %::server %::port %::pre %::c? %::text
}
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 11[12 $+ %::text $+ 11] }
%:echo 11!12 $+ %::nick $+ 11!12 %::text $+  %:comments | haltdef
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 11[12 $+ %::text $+ 11] }
%:echo 11-12 $+ %::server $+ 11-12 %::text $+  %:comments | haltdef
...
%:echo 11[ $+ %::cmode $+ 12 $+ %::nick $+ 11]12 %::text $+  %:comments
  }
  else {
    set -u1 %::cnick $cnick($me).color
%:echo 11(12 $+ %::nick $+ 11)12 %::text $+  %:comments
  }
...
else { %:echo 11(12 $+ %::nick $+ 11)12 %::text $+  %:comments }
  }
  else {
    set -u1 %:echo echo $color(own) -ti2 $+ $_active
%:echo 11-> *12 $+ %::target $+ 11*12 %::text $+  %:comments | haltdef
  }
...
else { %:echo 11* 12 $+ %::me %::text $+  %:comments }
  }
...
if ($1 ischan) { set -u1 %::chan $1 | %:echo 11-> -12 $+ %::target $+ 11-12 %::text $+  %:comments }
else { %:echo 11•12• notice sent 11» 12( $+ %::nick $+ )1112 %::text $+  %:comments }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 11[12 $+ %::text $+ 11] }
%:echo 11[ $+ %::cmode $+ 12 $+ %::nick $+ 11]12 %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
on &^*:ACTION:*:#:{
  if ($chan !ischan) { return }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 11[12 $+ %::text $+ 11] }
%:echo 11• 12 $+ %::nick %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
on &^*:NOTICE:*:#:{
  if ($chan !ischan) { return }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 11[12 $+ %::text $+ 11] }
%:echo 11-12 $+ %::nick $+ : $+ %::target $+ 11-12 %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 11[12 $+ %::text $+ 11] }
if ($query($nick)) { set -u1 %:echo %e $nick | %:echo 11* 12 $+ %::nick %::text $+  %:comments | haltdef }
  else {
    if ($window(Message Window)) { set -u1 %:echo %e $+ d }
    elseif ($_isactive) && (@* !iswm $active) && ($hget(Kte_Events, Active.Queries)) { set -u1 %:echo %e $+ a }
    else { set -u1 %:echo %e $+ s }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 11[12 $+ %::text $+ 11] }
%:echo 11-12 $+ %::nick 11[12 $+ %::address $+ 11]-12 %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::text
}
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 11[12 $+ %::text $+ 11] }
if (%ch) { %:echo 11[12 $+ %::nick $+ : $+ %::chan %::ctcp $+ 11] 12 $+ %::text $+  %:comments }
else { %:echo 11[12 $+ %::nick %::ctcp $+ 11] 12 $+ %::text $+  %:comments }
...
if ($1 ischan) { set -u1 %::chan $1 | %:echo 11-> [12 $+ %::target %::ctcp $+ 11] 12 $+ %::text $+  %:comments }
else { set -u1 %::nick $1 | %:echo 11-> [12 $+ %::target %::ctcp $+ 11] 12 $+ %::text $+  %:comments }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 11[12 $+ %::text $+ 11] }
%:echo 11[12 $+ %::nick %::ctcp reply11] 12 $+ %::text $+  %:comments | haltdef
...
set -nu1 %::c1 12 | set -nu1 %::c2 11 | set -nu1 %::c3 11 | set -nu1 %::c4 12
if (* iswm %::text) { set -nu1 %::parentext 11[12 $+ %::text $+ 11] }
%:echo 11-> [12 $+ %::target %::ctcp reply11] 12 $+ %::text $+  %:comments
...
alias -l kte::Echo %:echo 12 $+ %::text $+  %:comments
alias -l kte::EchoTarget %:echo 12 $+ %::text $+  %:comments
alias -l kte::Error %:echo     11»11» 1212error: %::text $+  %:comments
alias -l kte::Raw.303 %:echo     11»11» 12 %::text are online %:comments
alias -l kte::RAW.375 %:echo     11»11» 1212MOTD - %::server $+  %:comments
alias -l kte::RAW.255 %:echo     11»11» 1212I have %::users clients and %::value servers %:comments
alias -l kte::RAW.471 %:echo     11»11» 1212error:  $+ %::chan $+  is full %:comments
alias -l kte::RAW.391 %:echo     11»11» 1212server time: %::text $+  %:comments
alias -l kte::Raw.332 %:echo     11»11» 1212 topic: 11(11 $+ %::text $+ 11)12 $+  %:comments
alias -l kte::RAW.372 %:echo 12 $+ %::text $+  %:comments
alias -l kte::RAW.252 %:echo $+(%::pre,,%::c1,%::value) $iif(%::value == 1,operator,operators) online
alias -l kte::Raw.408 %:echo     11»11» 1212error:  $+ %::chan $+  you cannot use colors on this channel %:comments
...
alias -l kte::RAW.265 %:echo     11»11» 1212Current local users: %::users max: %::value $+  %:comments
alias -l kte::RAW.253 %:echo     11»11» 1212 $+ %::value unknown connections %:comments
alias -l kte::RAW.001 %:echo 12Welcome %::text $+  %:comments
alias -l kte::RAW.482 %:echo     11»11» 1212error:  $+ %::chan $+  you aren't an op %:comments
alias -l kte::RAW.401 %:echo     11»11» 1212error:  $+ %::nick $+  no such nickname %:comments
alias -l kte::RAW.266 %:echo     11»11» 1212Current global users: %::users max: %::value $+  %:comments
alias -l kte::RAW.250 %:echo     11»11» 1212 $+ %::value total connections %:comments
alias -l kte::RAW.002 %:echo 12Your Host Is %::server $+ , running %::value $+  %:comments
alias -l kte::RAW.474 %:echo     11»11» 1212error:  $+ %::chan $+  banned from channel %:comments
alias -l kte::RAW.003 %:echo 12This server was created on %::value $+  %:comments
alias -l kte::Raw.251 %:echo     11»11» 1212There are %::users users online, %::text invisible, on %::value servers %:comments
alias -l kte::RAW.475 %:echo     11»11» 1212error:  $+ %::chan $+  key required %:comments
alias -l kte::RAW.324 %:echo     11»11» 1212 modes: 11[11 $+ %::modes $+ 11]12 %:comments
alias -l kte::RAW.403 %:echo     11»11» 1212error:  $+ %::chan $+  no such channel %:comments
alias -l kte::RAW.376 %:echo     11»11» 1212end of MOTD %:comments
alias -l kte::RAW.221 %:echo     11»11» 1212 usermode: %::modes $+  %:comments
alias -l kte::Raw.301 %:echo     11»11» 12 %::nick is away: %::text $+  %:comments
alias -l kte::RAW.432 %:echo     11»11» 1212error:  $+ %::nick $+  invalid nickname %:comments
alias -l kte::RAW.404 %:echo     11»11» 1212error:  $+ %::chan $+  unable to send message %:comments
alias -l kte::RAW.353 %:echo     11»11» 1212names 11(11 $+ %::chan $+ 11):12 %::text $+  %:comments
alias -l kte::RAW.341 %:echo     11»11» 1212 $+ %::nick invited to %::chan $+  %:comments
alias -l kte::RAW.005 %:echo %::text $+  %:comments
alias -l kte::RAW.473 %:echo     11»11» 1212error:  $+ %::chan $+  invite only %:comments
alias -l kte::Raw.302 %:echo     11»11» 1212 userhost %::nick $+ : %::address $+  %:comments
alias -l kte::RAW.433 %:echo     11»11» 1212error:  $+ %::nick $+  is in use %:comments
alias -l kte::RAW.421 %:echo     11»11» 1212error:  $+ %::value $+  invalid command %:comments
alias -l kte::RAW.366 %:echo     11»11» 1212end of /names %::chan $+  %:comments
