...
set -nu1 %::pre 3[4!3]
set -nu1 %::c1 02 | set -nu1 %::c2 04 | set -nu1 %::c3 03 | set -nu1 %::c4 04
...
else { %:echo 3[4!3]02 %::nick 02(04 $+ $replace(%::address,@, $+ 02@ $+ %::c2) $+ 02) $+ 04 has joined02 %::chan | haltdef }
...
if ($nick($chan, $nick).pnick != $nick) { set -nu1 %::cmode $left($ifmatch, 1) } | set -nu1 %::cnick $nick($chan, $nick).color
set -nu1 %::chan $chan
if (* iswm %::text) { set -nu1 %::parentext 2[4 $+ %::text $+ 2] }
%:echo 3[4!3]02 %::nick 02(04 $+ $replace(%::address,@, $+ 02@ $+ %::c2) $+ 02) $+ 04 has left02 %::chan %::parentext | haltdef
...
if (* iswm %::text) { set -nu1 %::parentext 2[4 $+ %::text $+ 2] }
  while (%i) {
    ...
    if (%tgt = 3) || (%tgt = 2) { %st = 1 }
if (!%tgt) || (%tgt = 3) || (%tgt = 1) { %:echo = echo $color(quit) -ti2 %::chan | %:echo 3[4!3]02 %::nick 02(04 $+ $replace(%::address,@, $+ 02@ $+ %::c2) $+ 02) $+ 04 has quit IRC %::parentext }
    dec %i
  }
  unset %::chan
if ($query($nick)) { %:echo = echo $color(quit) -ti2 $nick | %:echo 3[4!3]02 %::nick 02(04 $+ $replace(%::address,@, $+ 02@ $+ %::c2) $+ 02) $+ 04 has quit IRC %::parentext }
if (%st) { %:echo = echo $color(quit) -sti2 | %:echo 3[4!3]02 %::nick 02(04 $+ $replace(%::address,@, $+ 02@ $+ %::c2) $+ 02) $+ 04 has quit IRC %::parentext }
  haltdef
...
if ($nick == $me) { %:echo = echo $color(nick) -sti2 | %:echo 3[4!3]02 You04 are now known as 02 $+ %::newnick $+  %:comments }
  while (%i) {
    set -u1 %::chan $comchan($newnick, %i)
    %tgt = $gettok($hget(Kte_Events, %::chan), 7, 44)
if (%tgt != 2) || ($nick == $me) { %:echo = echo $color(nick) -ti2 %::chan | %:echo 3[4!3] 02 $+ %::nick $+ 04 is now known as 02 $+ %::newnick $+  %:comments }
    dec %i
  }
if ($query($newnick)) { unset %::chan | %:echo = echo $color(nick) -ti2 $newnick | %:echo 3[4!3] 02 $+ %::nick $+ 04 is now known as 02 $+ %::newnick $+  %:comments }
  haltdef
...
%:echo 3[4!3] 04Usermode changed to02: %::modes $+  %:comments | haltdef
  unset %::modes
...
set -nu1 %::chan $chan
  ...
  set -nu1 %::modes $1-
%:echo 3[4!3] 02 $+ %::nick $+ 04 sets mode02: %::modes $+  %:comments | haltdef
  unset %::modes
unset %:echo %::nick %::address %::target %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick
}
...
set -nu1 %::chan $chan
if (* iswm %::text) { set -nu1 %::parentext 2[4 $+ %::text $+ 2] }
%:echo 3[4!3]02 %::nick $+ 04 changes topic to02: 04'02 $+ %::text $+ 04' %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
on ^&*:KICK:#:{
  if ($chan !ischan) { return }
...
if (* iswm %::text) { set -nu1 %::parentext 2[4 $+ %::text $+ 2] }
  if ($knick == $me) {
%:echo 3[4!3]02 You04 were kicked by 02 $+ %::nick $+ ,04 from 02 $+ %::chan %::parentext $+  %:comments
    if (%tgt == 2) { set -u1 %:echo echo $color(kick) -ti2 $chan }
    else { set -u1 %:echo echo $color(kick) -seti2 }
%:echo 3[4!3]02 You04 were kicked by 02 $+ %::nick $+ ,04 from 02 $+ %::chan %::parentext $+  %:comments
    _left $chan
  }
else { %:echo 3[4!3]02 %::knick $+ 04's ass was kicked by 02 $+ %::nick %::parentext $+  %:comments }
  haltdef
...
%:echo 3[4!3]02 %::nick $+ 04 has invited you to join 02 $+ %::chan $+  %:comments | haltdef
...
set -nu1 %::nick $nick
if (* iswm %::text) { set -nu1 %::parentext 2[4 $+ %::text $+ 2] }
%:echo 3[4!3]04 %::text $+  %:comments | haltdef
...
set -nu1 %::address $address
if (* iswm %::text) { set -nu1 %::parentext 2[4 $+ %::text $+ 2] }
%:echo 3[4!3] %::nick is on IRC %::parentext $+  %:comments | haltdef
unset %:echo %::nick %::address %::parentext %::me %::server %::port %::pre %::c? %::text
}
on ^&*:UNOTIFY:{
  set -u1 %:echo echo $color(notify) -ti2 $+ $iif(($_isactive) && (@* !iswm $active), a, s)
...
set -nu1 %::address $address
if (* iswm %::text) { set -nu1 %::parentext 2[4 $+ %::text $+ 2] }
%:echo 3[4!3] %::nick is offline %:comments | haltdef
unset %:echo %::nick %::address %::parentext %::me %::server %::port %::pre %::c? %::text
}
...
%:echo 3[4!3] 04Resolved 02 $+ %::address $+ 04 to 02 $+ %::raddress $+  %:comments | haltdef
      inc %i
    }
  }
  else {
    set -nu1 %::nick $nick
    set -nu1 %::address $address
%:echo 3[4!3] 04Unable to resolve 02 $+ %::address $+  %:comments | haltdef
  }
...
%:echo 02(04 $+ %::cmode $+ 04 $+ %::me $+ 02)04 %::text $+  %:comments
  }
  else {
    set -u1 %::cnick $cnick($me).color
%:echo 02(04 $+ %::me $+ 02)04 %::text $+  %:comments
  }
...
else { %:echo 02(04 $+ %::me $+ 02)04 %::text $+  %:comments }
  }
  else {
    set -u1 %:echo echo $color(own) -ti2 $+ $_active
%:echo 3[4!3] 02Msg (04 $+ %::target $+ 02)04 - %::text $+  %:comments | haltdef
  }
...
%:echo 02! 04 $+ %::cmode $+ 04 $+ %::me %::text $+  %:comments
...
else { %:echo 02! 04 $+ %::me %::text $+  %:comments }
  }
...
else { %:echo 3[4!3]02 Notice 04(02 $+ %::nick $+ 04) - %::text $+  %:comments }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 2[4 $+ %::text $+ 2] }
%:echo 02(04 $+ %::cmode $+ 03 $+ %::nick $+ 02)03 %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
on &^*:ACTION:*:#:{
  if ($chan !ischan) { return }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 2[4 $+ %::text $+ 2] }
%:echo 02! 04 $+ %::cmode $+ 03 $+ %::nick %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
on &^*:NOTICE:*:#:{
  if ($chan !ischan) { return }
...
if (* iswm %::text) { set -nu1 %::parentext 2[4 $+ %::text $+ 2] }
%:echo 02! 03 $+ %::nick %::text $+  %:comments | haltdef
  }
  else {
    set -u1 %:echo echo $color(normal) -mti2lbf =$nick
    set -nu1 %::text $strip($1-, o)
if (* iswm %::text) { set -nu1 %::parentext 2[4 $+ %::text $+ 2] }
%:echo 02(03 $+ %::nick $+ 02)03 %::text $+  %:comments | haltdef
  }
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::text
}
...
%:echo 02[04 $+ %::nick $+ 02]04 - %::text $+  %:comments | haltdef
  }
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::text
}
on &^*:ACTION:*:?:{
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 2[4 $+ %::text $+ 2] }
if ($query($nick)) { set -u1 %:echo %e $nick | %:echo 02! 03 $+ %::nick %::text $+  %:comments | haltdef }
  else {
    if ($window(Message Window)) { set -u1 %:echo %e $+ d }
    elseif ($_isactive) && (@* !iswm $active) && ($hget(Kte_Events, Active.Queries)) { set -u1 %:echo %e $+ a }
    else { set -u1 %:echo %e $+ s }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 2[4 $+ %::text $+ 2] }
%:echo 02 $+ %::nick $+ 04 - %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::text
}
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 2[4 $+ %::text $+ 2] }
if (%ch) { %:echo 3[4!3] 03 $+ %::nick $+ 04/03 $+ %::chan 04- CTCP %::text $+  %:comments }
else { %:echo 3[4!3] 03 $+ %::nick (04 $+ %::address $+ 03)04 - CTCP %::text $+  %:comments }
...
if ($1 ischan) { set -u1 %::chan $1 | %:echo 3[4!3]03 Ctcp (04 $+ %::target $+ 03)04 - %::text $+  %:comments }
else { set -u1 %::nick $1 | %:echo 3[4!3]03 Ctcp (04 $+ %::target $+ 03)04 - %::text $+  %:comments }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 2[4 $+ %::text $+ 2] }
%:echo 3[4!3] 03 $+ %::nick (04 $+ %::address $+ 03)04 - CTCP REPLY %::text $+  %:comments | haltdef
...
set -nu1 %::c1 02 | set -nu1 %::c2 04 | set -nu1 %::c3 03 | set -nu1 %::c4 04
if (* iswm %::text) { set -nu1 %::parentext 2[4 $+ %::text $+ 2] }
%:echo 3[4!3] 03Ctcp (04 $+ %::target $+ 03)04 - REPLY %::text $+  %:comments
...
set -nu1 %::pre 3[4!3]
set -nu1 %::c1 02 | set -nu1 %::c2 04 | set -nu1 %::c3 03 | set -nu1 %::c4 04
%:echo 3[4!3] 04Looking up 02 $+ %::address $+ 04... %:comments
...
alias -l kte::Echo %:echo 3[4!3]04 %::text $+  %:comments
alias -l kte::EchoTarget %:echo 3[4!3] 02[04 $+ %::target $+ 02]04 %::text $+  %:comments
alias -l kte::Error %:echo 3[4!3]04 %::text $+  %:comments
alias -l kte::RAW.311 e.whois
alias -l kte::Raw.303 %:echo 3[4!3] %::text are online %:comments
alias -l kte::RAW.375 %:echo 3[4!3]04 Message of the day02: %:comments
alias -l kte::RAW.319 e.chans
alias -l kte::RAW.255 %:echo 3[4!3]04 There are02 %::users $+ 04 clients on 02 $+ %::value $+ 04 server(s) $+  %:comments
alias -l kte::RAW.471 %:echo 3[4!3]04 Can't join 02 $+ %::chan $+ 04, because it's full (02mode +l04) $+  %:comments
alias -l kte::RAW.391 %:echo 3[4!3]04 Date02: $asctime($ctime(%::text)) %:comments
alias -l kte::Raw.332 e.topic
alias -l kte::RAW.372 %:echo 04 $+ %::text $+  %:comments
alias -l kte::RAW.252 %:echo 3[4!3]02 %::value $+ 04 operator(s) online %:comments
alias -l kte::RAW.333 e.settopic
alias -l kte::RAW.317 e.idle
alias -l kte::RAW.329 %:echo 3[4!3]04 Created on02: $asctime(%::text) %:comments
alias -l kte::RAW.265 %:echo 3[4!3]04 Local users02: %::users 04Max02: %::value $+  %:comments
alias -l kte::RAW.253 %:echo 3[4!3]04 Unknown connection(s)02: %::value $+  %:comments
alias -l kte::RAW.001 %:echo 3[4!3]04 %::text $+  %:comments
alias -l kte::RAW.482 %:echo 3[4!3]04 You're not operator on 02 $+ %::chan $+  %:comments
alias -l kte::RAW.401 %:echo 3[4!3]04 No such nickname02: %::nick $+  %:comments
alias -l kte::RAW.266 e.lusers.end
alias -l kte::RAW.250 %:echo 3[4!3]04 Total connection(s)02: %::value $+  %:comments
alias -l kte::RAW.002 %:echo 3[4!3]04 Host server02: %::server 04running version 02 $+ %::value $+  %:comments
alias -l kte::RAW.474 %:echo 3[4!3]04 Can't join 02 $+ %::chan $+ 04, because you're banned! (02mode +b04) $+  %:comments
alias -l kte::RAW.315 %:echo 3[4!3]04 End of 02/who04 list for02 %::chan $+  %:comments
alias -l kte::RAW.307 %:echo 3[4!3]02 Status04: %::isregd registered nickname %:comments
alias -l kte::RAW.003 %:echo 3[4!3]04 Server created on02: %::value $+  %:comments
alias -l kte::Raw.251 e.lusers
alias -l kte::RAW.475 %:echo 3[4!3]04 Can't join 02 $+ %::chan $+ 04, because it's requires key. Use 02/join %::chan >key< 04(02mode +k04) $+  %:comments
alias -l kte::RAW.324 %:echo 3[4!3]04 Current channel modes02: %::modes $+  %:comments
alias -l kte::RAW.312 %:echo 3[4!3]02 Server04: %::wserver 02(04 $+ %::serverinfo $+ 02) $+  %:comments
alias -l kte::RAW.403 %:echo 3[4!3]04 No such channel02: %::chan $+  %:comments
alias -l kte::RAW.376 %:echo 3[4!3]04 End of 02/motd04 command. %:comments
alias -l kte::RAW.352 %:echo 3[4!3]04 %::cmode $+ 03 $+ %::nick (04 $+ %::address $+ 03)04 %::realname $+  %:comments
alias -l kte::RAW.313 %:echo 3[4!3]02 IRCop04: %::isoper $+ 02/04 $+ %::operline $+  %:comments
alias -l kte::Raw.301 %:echo 3[4!3]02 Away04: %::text $+  %:comments
alias -l kte::RAW.404 %:echo 3[4!3]04 Unable to send message to 02 $+ %::chan $+  %:comments
alias -l kte::RAW.353 %:echo 3[4!3]04 %::chan $+ 02: %::text $+  %:comments
alias -l kte::RAW.341 %:echo 3[4!3]02 %::nick $+ 04 has been invited to join 02 $+ %::chan $+  %:comments
alias -l kte::RAW.005 %:echo 3[4!3]04 Protocols supported by this server02: %::text $+  %:comments
alias -l kte::RAW.473 %:echo 3[4!3]04 Can't join 02 $+ %::chan $+ 04, because it's invite only! (02mode +i04) $+  %:comments
alias -l kte::Raw.302 %:echo 3[4!3]04 Host02: %::nick $+ ! $+ %::address $+  %:comments
alias -l kte::RAW.433 %:echo 3[4!3]02 %::nick $+ 04 is already in use. Try 02[ $+ %::nick $+ ]] $+  %:comments
alias -l kte::RAW.421 %:echo 3[4!3]04 Invalid command02: %::value $+  %:comments
alias -l kte::RAW.366 %:echo 3[4!3]04 End of 02/names04 list for02 %::chan $+  %:comments
alias -l kte::RAW.318 e.whois.end
alias -l kte::RAW.254 %:echo 3[4!3]02 %::value 04 channels formed %:comments
