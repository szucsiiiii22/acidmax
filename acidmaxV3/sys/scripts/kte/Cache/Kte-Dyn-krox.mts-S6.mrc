...
; Kte-Dyn 1.5 FF4BFBB0 C89A4173
alias theme.text {
  if (!$var(%:echo)) { return $false }
set -nu1 %::me $me | set -nu1 %::server $server | set -nu1 %::port $port
set -nu1 %::pre     12»12» 02
set -nu1 %::c1 02 | set -nu1 %::c2 12 | set -nu1 %::c3 12 | set -nu1 %::c4 02
...
if ($nick == $me) { %:echo     12»12» 0202 [12 $+ %::chan $+ 02] %:comments | haltdef }
...
if ($nick($chan, $nick).pnick != $nick) { set -nu1 %::cmode $left($ifmatch, 1) } | set -nu1 %::cnick $nick($chan, $nick).color
set -nu1 %::chan $chan
if (* iswm %::text) { set -nu1 %::parentext 12[02 $+ %::text $+ 12] }
%:echo     12»12» 0202 part: %::nick 12[12 $+ %::address $+ 12] (12 $+ %::text $+ 12) $+  %:comments | haltdef
...
if ($nick == $me) { %:echo = echo $color(nick) -sti2 | %:echo     12»12» 0202 nick: %::newnick $+  %:comments }
  while (%i) {
    set -u1 %::chan $comchan($newnick, %i)
    %tgt = $gettok($hget(Kte_Events, %::chan), 7, 44)
if (%tgt != 2) || ($nick == $me) { %:echo = echo $color(nick) -ti2 %::chan | %:echo     12»12» 0202 nick: %::nick 12[12 $+ %::address $+ 12]02 is now %::newnick $+  %:comments }
    dec %i
  }
if ($query($newnick)) { unset %::chan | %:echo = echo $color(nick) -ti2 $newnick | %:echo     12»12» 0202 nick: %::nick 12[12 $+ %::address $+ 12]02 is now %::newnick $+  %:comments }
  haltdef
...
%:echo     12»12» 0202 usermode: %::modes $+  %:comments | haltdef
  unset %::modes
...
set -nu1 %::chan $chan
  ...
  set -nu1 %::modes $1-
%:echo     12»12» 0202 mode: %::nick 12[12 $+ %::address $+ 12] 02set: %::modes $+  %:comments | haltdef
  unset %::modes
unset %:echo %::nick %::address %::target %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick
}
...
set -nu1 %::chan $chan
if (* iswm %::text) { set -nu1 %::parentext 12[02 $+ %::text $+ 12] }
%:echo     12»12» 0202 %::nick sets topic: 12(12 $+ %::text $+ 12) $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
on ^&*:KICK:#:{
  if ($chan !ischan) { return }
...
if (* iswm %::text) { set -nu1 %::parentext 12[02 $+ %::text $+ 12] }
  if ($knick == $me) {
%:echo     12»12» 0202 You were kicked from %::chan by %::nick 12(12 $+ %::text $+ 12) $+  %:comments
    if (%tgt == 2) { set -u1 %:echo echo $color(kick) -ti2 $chan }
    else { set -u1 %:echo echo $color(kick) -seti2 }
%:echo     12»12» 0202 You were kicked from %::chan by %::nick 12(12 $+ %::text $+ 12) $+  %:comments
    _left $chan
  }
else { %:echo     12»12» 0202 kick: %::nick 12[12 $+ %::address $+ 12]02 kicked %::knick 12(12 $+ %::text $+ 12) $+  %:comments }
  haltdef
...
%:echo     12»12» 0202 invite: %::nick 12(12 $+ %::address $+ 12)02 invites you to join %::chan $+  %:comments | haltdef
...
set -nu1 %::nick $nick
if (* iswm %::text) { set -nu1 %::parentext 12[02 $+ %::text $+ 12] }
%:echo     12»12» 0202 error: %::text $+  %:comments | haltdef
...
set -nu1 %::address $address
if (* iswm %::text) { set -nu1 %::parentext 12[02 $+ %::text $+ 12] }
%:echo     12»12» 0202 nick: %::nick 12(12 $+ %::address $+ 12) is on irc %:comments | haltdef
unset %:echo %::nick %::address %::parentext %::me %::server %::port %::pre %::c? %::text
}
on ^&*:UNOTIFY:{
  set -u1 %:echo echo $color(notify) -ti2 $+ $iif(($_isactive) && (@* !iswm $active), a, s)
...
set -nu1 %::address $address
if (* iswm %::text) { set -nu1 %::parentext 12[02 $+ %::text $+ 12] }
%:echo     12»12» 0202 nick: %::nick 12(12 $+ %::address $+ 12) is off irc %:comments | haltdef
unset %:echo %::nick %::address %::parentext %::me %::server %::port %::pre %::c? %::text
}
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 12[02 $+ %::text $+ 12] }
%:echo 12!02 $+ %::nick $+ 12!02 %::text $+  %:comments | haltdef
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 12[02 $+ %::text $+ 12] }
%:echo 12-02 $+ %::server $+ 12-02 %::text $+  %:comments | haltdef
...
%:echo 12[ $+ %::cmode $+ 02 $+ %::nick $+ 12]02 %::text $+  %:comments
  }
  else {
    set -u1 %::cnick $cnick($me).color
%:echo 12(02 $+ %::nick $+ 12)02 %::text $+  %:comments
  }
...
else { %:echo 12(02 $+ %::nick $+ 12)02 %::text $+  %:comments }
  }
  else {
    set -u1 %:echo echo $color(own) -ti2 $+ $_active
%:echo 12-> *02 $+ %::target $+ 12*02 %::text $+  %:comments | haltdef
  }
...
else { %:echo 12* 02 $+ %::me %::text $+  %:comments }
  }
...
if ($1 ischan) { set -u1 %::chan $1 | %:echo 12-> -02 $+ %::target $+ 12-02 %::text $+  %:comments }
else { %:echo 12•02• notice sent 12» 02( $+ %::nick $+ )1202 %::text $+  %:comments }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 12[02 $+ %::text $+ 12] }
%:echo 12[ $+ %::cmode $+ 02 $+ %::nick $+ 12]02 %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
on &^*:ACTION:*:#:{
  if ($chan !ischan) { return }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 12[02 $+ %::text $+ 12] }
%:echo 12• 02 $+ %::nick %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
on &^*:NOTICE:*:#:{
  if ($chan !ischan) { return }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 12[02 $+ %::text $+ 12] }
%:echo 12-02 $+ %::nick $+ : $+ %::target $+ 12-02 %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 12[02 $+ %::text $+ 12] }
if ($query($nick)) { set -u1 %:echo %e $nick | %:echo 12* 02 $+ %::nick %::text $+  %:comments | haltdef }
  else {
    if ($window(Message Window)) { set -u1 %:echo %e $+ d }
    elseif ($_isactive) && (@* !iswm $active) && ($hget(Kte_Events, Active.Queries)) { set -u1 %:echo %e $+ a }
    else { set -u1 %:echo %e $+ s }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 12[02 $+ %::text $+ 12] }
%:echo 12-02 $+ %::nick 12[02 $+ %::address $+ 12]-02 %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::text
}
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 12[02 $+ %::text $+ 12] }
if (%ch) { %:echo 12[02 $+ %::nick $+ : $+ %::chan %::ctcp $+ 12] 02 $+ %::text $+  %:comments }
else { %:echo 12[02 $+ %::nick %::ctcp $+ 12] 02 $+ %::text $+  %:comments }
...
if ($1 ischan) { set -u1 %::chan $1 | %:echo 12-> [02 $+ %::target %::ctcp $+ 12] 02 $+ %::text $+  %:comments }
else { set -u1 %::nick $1 | %:echo 12-> [02 $+ %::target %::ctcp $+ 12] 02 $+ %::text $+  %:comments }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 12[02 $+ %::text $+ 12] }
%:echo 12[02 $+ %::nick %::ctcp reply12] 02 $+ %::text $+  %:comments | haltdef
...
set -nu1 %::c1 02 | set -nu1 %::c2 12 | set -nu1 %::c3 12 | set -nu1 %::c4 02
if (* iswm %::text) { set -nu1 %::parentext 12[02 $+ %::text $+ 12] }
%:echo 12-> [02 $+ %::target %::ctcp reply12] 02 $+ %::text $+  %:comments
...
alias -l kte::Error %:echo     12»12» 0202error: %::text $+  %:comments
alias -l kte::Raw.303 %:echo     12»12» 02 %::text are online %:comments
alias -l kte::RAW.375 %:echo     12»12» 0202MOTD - %::server $+  %:comments
alias -l kte::RAW.255 %:echo     12»12» 0202I have %::users clients and %::value servers %:comments
alias -l kte::RAW.471 %:echo     12»12» 0202error:  $+ %::chan $+  is full %:comments
alias -l kte::RAW.391 %:echo     12»12» 0202server time: %::text $+  %:comments
alias -l kte::Raw.332 %:echo     12»12» 0202 topic: 12(12 $+ %::text $+ 12)02 $+  %:comments
alias -l kte::RAW.372 %:echo 02 $+ %::text $+  %:comments
alias -l kte::RAW.252 %:echo $+(%::pre,,%::c1,%::value) $iif(%::value == 1,operator,operators) online
alias -l kte::Raw.408 %:echo     12»12» 0202error:  $+ %::chan $+  you cannot use colors on this channel %:comments
...
alias -l kte::RAW.265 %:echo     12»12» 0202Current local users: %::users max: %::value $+  %:comments
alias -l kte::RAW.253 %:echo     12»12» 0202 $+ %::value unknown connections %:comments
alias -l kte::RAW.001 %:echo 02Welcome %::text $+  %:comments
alias -l kte::RAW.482 %:echo     12»12» 0202error:  $+ %::chan $+  you aren't an op %:comments
alias -l kte::RAW.401 %:echo     12»12» 0202error:  $+ %::nick $+  no such nickname %:comments
alias -l kte::RAW.266 %:echo     12»12» 0202Current global users: %::users max: %::value $+  %:comments
alias -l kte::RAW.250 %:echo     12»12» 0202 $+ %::value total connections %:comments
alias -l kte::RAW.002 %:echo 02Your Host Is %::server $+ , running %::value $+  %:comments
alias -l kte::RAW.474 %:echo     12»12» 0202error:  $+ %::chan $+  banned from channel %:comments
alias -l kte::RAW.003 %:echo 02This server was created on %::value $+  %:comments
alias -l kte::Raw.251 %:echo     12»12» 0202There are %::users users online, %::text invisible, on %::value servers %:comments
alias -l kte::RAW.475 %:echo     12»12» 0202error:  $+ %::chan $+  key required %:comments
alias -l kte::RAW.324 %:echo     12»12» 0202 modes: 12[12 $+ %::modes $+ 12]02 %:comments
alias -l kte::RAW.403 %:echo     12»12» 0202error:  $+ %::chan $+  no such channel %:comments
alias -l kte::RAW.376 %:echo     12»12» 0202end of MOTD %:comments
alias -l kte::RAW.221 %:echo     12»12» 0202 usermode: %::modes $+  %:comments
alias -l kte::Raw.301 %:echo     12»12» 02 %::nick is away: %::text $+  %:comments
alias -l kte::RAW.432 %:echo     12»12» 0202error:  $+ %::nick $+  invalid nickname %:comments
alias -l kte::RAW.404 %:echo     12»12» 0202error:  $+ %::chan $+  unable to send message %:comments
alias -l kte::RAW.353 %:echo     12»12» 0202names 12(12 $+ %::chan $+ 12):02 %::text $+  %:comments
alias -l kte::RAW.341 %:echo     12»12» 0202 $+ %::nick invited to %::chan $+  %:comments
alias -l kte::RAW.005 %:echo %::text $+  %:comments
alias -l kte::RAW.473 %:echo     12»12» 0202error:  $+ %::chan $+  invite only %:comments
alias -l kte::Raw.302 %:echo     12»12» 0202 userhost %::nick $+ : %::address $+  %:comments
alias -l kte::RAW.433 %:echo     12»12» 0202error:  $+ %::nick $+  is in use %:comments
alias -l kte::RAW.421 %:echo     12»12» 0202error:  $+ %::value $+  invalid command %:comments
alias -l kte::RAW.366 %:echo     12»12» 0202end of /names %::chan $+  %:comments
