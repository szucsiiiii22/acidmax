...
if ($nick == $me) { %:echo     11»12» 0101 [11 $+ %::chan $+ 01] %:comments | haltdef }
...
%:echo     11»12» 0101 part: %::nick 12[11 $+ %::address $+ 12] [11 $+ %::text $+ 12] %:comments | haltdef
...
set -nu1 %::address $address
if (* iswm %::text) { set -nu1 %::parentext 11[01 $+ %::text $+ 11] }
%:echo     11»12» 0101 nick: %::nick 12[11 $+ %::address $+ 12] is on irc %:comments | haltdef
unset %:echo %::nick %::address %::parentext %::me %::server %::port %::pre %::c? %::text
}
on ^&*:UNOTIFY:{
  set -u1 %:echo echo $color(notify) -ti2 $+ $iif(($_isactive) && (@* !iswm $active), a, s)
...
set -nu1 %::address $address
if (* iswm %::text) { set -nu1 %::parentext 11[01 $+ %::text $+ 11] }
%:echo     11»12» 0101 nick: %::nick 12[11 $+ %::address $+ 12] is off irc %:comments | haltdef
unset %:echo %::nick %::address %::parentext %::me %::server %::port %::pre %::c? %::text
}
...
%:echo 11[ $+ %::cmode $+ 01 $+ %::nick $+ 11]01 %::text $+  %:comments
  }
  else {
    set -u1 %::cnick $cnick($me).color
%:echo 11[01 $+ %::nick $+ 11]01 %::text $+  %:comments
  }
...
else { %:echo 11[01 $+ %::nick $+ 11]01 %::text $+  %:comments }
  }
  else {
    set -u1 %:echo echo $color(own) -ti2 $+ $_active
%:echo 11-> *01 $+ %::target $+ 11*01 %::text $+  %:comments | haltdef
  }
...
else { %:echo 11* 01 $+ %::me %::text $+  %:comments }
  }
...
if ($1 ischan) { set -u1 %::chan $1 | %:echo 11-> -01 $+ %::target $+ 11-01 %::text $+  %:comments }
else { %:echo 11•01• notice sent 11» 01( $+ %::nick $+ )1101 %::text $+  %:comments }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 11[01 $+ %::text $+ 11] }
%:echo 11[ $+ %::cmode $+ 01 $+ %::nick $+ 11]01 %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
on &^*:ACTION:*:#:{
  if ($chan !ischan) { return }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 11[01 $+ %::text $+ 11] }
%:echo 11• 01 $+ %::nick %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
on &^*:NOTICE:*:#:{
  if ($chan !ischan) { return }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 11[01 $+ %::text $+ 11] }
%:echo 11-01 $+ %::nick $+ : $+ %::target $+ 11-01 %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 11[01 $+ %::text $+ 11] }
if ($query($nick)) { set -u1 %:echo %e $nick | %:echo 11* 01 $+ %::nick %::text $+  %:comments | haltdef }
  else {
    if ($window(Message Window)) { set -u1 %:echo %e $+ d }
    elseif ($_isactive) && (@* !iswm $active) && ($hget(Kte_Events, Active.Queries)) { set -u1 %:echo %e $+ a }
    else { set -u1 %:echo %e $+ s }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 11[01 $+ %::text $+ 11] }
%:echo 11-01 $+ %::nick 11[01 $+ %::address $+ 11]-01 %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::text
}
...
alias -l kte::RAW.471 %:echo     11»12» 0101error:  $+ %::chan $+  is full %:comments
alias -l kte::RAW.391 %:echo     11»12» 0101server time: %::text $+  %:comments
alias -l kte::Raw.332 %:echo 12.::01 topic: 12[11 $+ %::text $+ 12]01 %:comments
alias -l kte::RAW.372 %:echo 01 $+ %::text $+  %:comments
alias -l kte::RAW.252 %:echo $+(%::pre,,%::c1,%::value) $iif(%::value == 1,operator,operators) online
alias -l kte::Raw.408 %:echo     11»12» 0101error:  $+ %::chan $+  you cannot use colors on this channel %:comments
...
alias -l kte::RAW.265 %:echo     11»12» 0101Current local users: %::users max: %::value $+  %:comments
alias -l kte::RAW.253 %:echo     11»12» 0101 $+ %::value unknown connections %:comments
alias -l kte::RAW.001 %:echo 01Welcome %::text $+  %:comments
alias -l kte::RAW.482 %:echo     11»12» 0101error:  $+ %::chan $+  you aren't an op %:comments
alias -l kte::RAW.401 %:echo     11»12» 0101error:  $+ %::nick $+  no such nickname %:comments
alias -l kte::RAW.266 %:echo     11»12» 0101Current global users: %::users max: %::value $+  %:comments
...
alias -l kte::RAW.474 %:echo     11»12» 0101error:  $+ %::chan $+  banned from channel %:comments
...
alias -l kte::RAW.475 %:echo     11»12» 0101error:  $+ %::chan $+  key required %:comments
alias -l kte::RAW.324 %:echo 12.::01 modes: 12[11 $+ %::modes $+ 12]01 %:comments
alias -l kte::RAW.403 %:echo     11»12» 0101error:  $+ %::chan $+  no such channel %:comments
alias -l kte::RAW.376 %:echo     11»12» 0101end of MOTD %:comments
alias -l kte::RAW.313 %:echo     11»12» 01 %::text $+  %:comments
alias -l kte::RAW.221 %:echo     11»12» 0101 usermode: %::modes $+  %:comments
alias -l kte::Raw.301 %:echo     11»12» 01 %::nick is away: %::text $+  %:comments
alias -l kte::RAW.432 %:echo     11»12» 0101error:  $+ %::nick $+  invalid nickname %:comments
alias -l kte::RAW.404 %:echo     11»12» 0101error:  $+ %::chan $+  unable to send message %:comments
alias -l kte::RAW.353 %:echo 12.::01 names 12[11 $+ %::chan $+ 12]:01 %::text $+  %:comments
alias -l kte::RAW.341 %:echo     11»12» 0101 $+ %::nick invited to %::chan $+  %:comments
alias -l kte::RAW.005 %:echo %::text $+  %:comments
alias -l kte::RAW.473 %:echo     11»12» 0101error:  $+ %::chan $+  invite only %:comments
alias -l kte::Raw.302 %:echo     11»12» 0101 userhost %::nick $+ : %::address $+  %:comments
alias -l kte::RAW.433 %:echo     11»12» 0101error:  $+ %::nick $+  is in use %:comments
alias -l kte::RAW.421 %:echo     11»12» 0101error:  $+ %::value $+  invalid command %:comments
alias -l kte::RAW.366 %:echo 12.::01 end of /names %::chan $+  %:comments
