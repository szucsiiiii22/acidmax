...
; Kte-Dyn 1.5 FF4BFBB0 DFAB4B2E
alias theme.text {
  if (!$var(%:echo)) { return $false }
set -nu1 %::me $me | set -nu1 %::server $server | set -nu1 %::port $port
set -nu1 %::pre 04[08:04]
set -nu1 %::c1 08 | set -nu1 %::c2 06 | set -nu1 %::c3 07 | set -nu1 %::c4 04
...
if ($nick == $me) { %:echo 04[08:04]08 [06 $+ %::chan $+ 08] %:comments | haltdef }
...
if ($nick($chan, $nick).pnick != $nick) { set -nu1 %::cmode $left($ifmatch, 1) } | set -nu1 %::cnick $nick($chan, $nick).color
set -nu1 %::chan $chan
if (* iswm %::text) { set -nu1 %::parentext 06[08 $+ %::text $+ 06] }
%:echo 04[08:04]08 part: %::nick 07[06 $+ %::address $+ 07] (06 $+ %::text $+ 07) $+  %:comments | haltdef
...
if ($nick == $me) { %:echo = echo $color(nick) -sti2 | %:echo 04[08:04]08 nick: %::newnick $+  %:comments }
  while (%i) {
    set -u1 %::chan $comchan($newnick, %i)
    %tgt = $gettok($hget(Kte_Events, %::chan), 7, 44)
if (%tgt != 2) || ($nick == $me) { %:echo = echo $color(nick) -ti2 %::chan | %:echo 04[08:04]08 nick: %::nick 07[06 $+ %::address $+ 07]08 is now %::newnick $+  %:comments }
    dec %i
  }
if ($query($newnick)) { unset %::chan | %:echo = echo $color(nick) -ti2 $newnick | %:echo 04[08:04]08 nick: %::nick 07[06 $+ %::address $+ 07]08 is now %::newnick $+  %:comments }
  haltdef
...
%:echo 04[08:04]08 usermode: %::modes $+  %:comments | haltdef
  unset %::modes
...
set -nu1 %::chan $chan
  ...
  set -nu1 %::modes $1-
%:echo 04[08:04]08 mode: %::nick 07[06 $+ %::address $+ 07] 08set: %::modes $+  %:comments | haltdef
  unset %::modes
unset %:echo %::nick %::address %::target %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick
}
...
set -nu1 %::chan $chan
if (* iswm %::text) { set -nu1 %::parentext 06[08 $+ %::text $+ 06] }
%:echo 04[08:04]08 %::nick sets topic: 07(06 $+ %::text $+ 07) $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
on ^&*:KICK:#:{
  if ($chan !ischan) { return }
...
if (* iswm %::text) { set -nu1 %::parentext 06[08 $+ %::text $+ 06] }
  if ($knick == $me) {
%:echo 04[08:04]08 You were kicked from %::chan by %::nick 07(06 $+ %::text $+ 07) $+  %:comments
    if (%tgt == 2) { set -u1 %:echo echo $color(kick) -ti2 $chan }
    else { set -u1 %:echo echo $color(kick) -seti2 }
%:echo 04[08:04]08 You were kicked from %::chan by %::nick 07(06 $+ %::text $+ 07) $+  %:comments
    _left $chan
  }
else { %:echo 04[08:04]08 kick: %::nick 07[06 $+ %::address $+ 07]08 kicked %::knick 07(06 $+ %::text $+ 07) $+  %:comments }
  haltdef
...
%:echo 04[08:04]08 invite: %::nick 07(06 $+ %::address $+ 07)08 invites you to join %::chan $+  %:comments | haltdef
...
set -nu1 %::nick $nick
if (* iswm %::text) { set -nu1 %::parentext 06[08 $+ %::text $+ 06] }
%:echo 04[08:04]08 error: %::text $+  %:comments | haltdef
...
set -nu1 %::address $address
if (* iswm %::text) { set -nu1 %::parentext 06[08 $+ %::text $+ 06] }
%:echo 04[08:04]08 nick: %::nick 07(06 $+ %::address $+ 07) is on irc %:comments | haltdef
unset %:echo %::nick %::address %::parentext %::me %::server %::port %::pre %::c? %::text
}
on ^&*:UNOTIFY:{
  set -u1 %:echo echo $color(notify) -ti2 $+ $iif(($_isactive) && (@* !iswm $active), a, s)
...
set -nu1 %::address $address
if (* iswm %::text) { set -nu1 %::parentext 06[08 $+ %::text $+ 06] }
%:echo 04[08:04]08 nick: %::nick 07(06 $+ %::address $+ 07) is off irc %:comments | haltdef
unset %:echo %::nick %::address %::parentext %::me %::server %::port %::pre %::c? %::text
}
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 06[08 $+ %::text $+ 06] }
%:echo 06!08 $+ %::nick $+ 06!08 %::text $+  %:comments | haltdef
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 06[08 $+ %::text $+ 06] }
%:echo 06-08 $+ %::server $+ 06-08 %::text $+  %:comments | haltdef
...
%:echo 06[ $+ %::cmode $+ 08 $+ %::nick $+ 06]08 %::text $+  %:comments
  }
  else {
    set -u1 %::cnick $cnick($me).color
%:echo 06(08 $+ %::nick $+ 06)08 %::text $+  %:comments
  }
...
else { %:echo 06(08 $+ %::nick $+ 06)08 %::text $+  %:comments }
  }
  else {
    set -u1 %:echo echo $color(own) -ti2 $+ $_active
%:echo 06-> *08 $+ %::target $+ 06*08 %::text $+  %:comments | haltdef
  }
...
else { %:echo 06* 08 $+ %::me %::text $+  %:comments }
  }
...
if ($1 ischan) { set -u1 %::chan $1 | %:echo 06-> -08 $+ %::target $+ 06-08 %::text $+  %:comments }
else { %:echo 06•08• notice sent 06» 08( $+ %::nick $+ )0608 %::text $+  %:comments }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 06[08 $+ %::text $+ 06] }
%:echo 06[ $+ %::cmode $+ 08 $+ %::nick $+ 06]08 %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
on &^*:ACTION:*:#:{
  if ($chan !ischan) { return }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 06[08 $+ %::text $+ 06] }
%:echo 06• 08 $+ %::nick %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
on &^*:NOTICE:*:#:{
  if ($chan !ischan) { return }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 06[08 $+ %::text $+ 06] }
%:echo 06-08 $+ %::nick $+ : $+ %::target $+ 06-08 %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 06[08 $+ %::text $+ 06] }
if ($query($nick)) { set -u1 %:echo %e $nick | %:echo 06* 08 $+ %::nick %::text $+  %:comments | haltdef }
  else {
    if ($window(Message Window)) { set -u1 %:echo %e $+ d }
    elseif ($_isactive) && (@* !iswm $active) && ($hget(Kte_Events, Active.Queries)) { set -u1 %:echo %e $+ a }
    else { set -u1 %:echo %e $+ s }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 06[08 $+ %::text $+ 06] }
%:echo 06-08 $+ %::nick 06[08 $+ %::address $+ 06]-08 %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::text
}
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 06[08 $+ %::text $+ 06] }
if (%ch) { %:echo 06[08 $+ %::nick $+ : $+ %::chan %::ctcp $+ 06] 08 $+ %::text $+  %:comments }
else { %:echo 06[08 $+ %::nick %::ctcp $+ 06] 08 $+ %::text $+  %:comments }
...
if ($1 ischan) { set -u1 %::chan $1 | %:echo 06-> [08 $+ %::target %::ctcp $+ 06] 08 $+ %::text $+  %:comments }
else { set -u1 %::nick $1 | %:echo 06-> [08 $+ %::target %::ctcp $+ 06] 08 $+ %::text $+  %:comments }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 06[08 $+ %::text $+ 06] }
%:echo 06[08 $+ %::nick %::ctcp reply06] 08 $+ %::text $+  %:comments | haltdef
...
set -nu1 %::c1 08 | set -nu1 %::c2 06 | set -nu1 %::c3 07 | set -nu1 %::c4 04
if (* iswm %::text) { set -nu1 %::parentext 06[08 $+ %::text $+ 06] }
%:echo 06-> [08 $+ %::target %::ctcp reply06] 08 $+ %::text $+  %:comments
...
alias -l kte::Echo %:echo 08 $+ %::text $+  %:comments
alias -l kte::EchoTarget %:echo 08 $+ %::text $+  %:comments
alias -l kte::Error %:echo 04[08:04]08error: %::text $+  %:comments
alias -l kte::Raw.303 %:echo 04[08:04] %::text are online %:comments
alias -l kte::RAW.375 %:echo 04[08:04]08MOTD - %::server $+  %:comments
alias -l kte::RAW.255 %:echo 04[08:04]08I have %::users clients and %::value servers %:comments
alias -l kte::RAW.471 %:echo 04[08:04]08error:  $+ %::chan $+  is full %:comments
alias -l kte::RAW.391 %:echo 04[08:04]08server time: %::text $+  %:comments
alias -l kte::Raw.332 %:echo 04[08:04]08 topic: 07(06 $+ %::text $+ 07)08 $+  %:comments
alias -l kte::RAW.372 %:echo 08 $+ %::text $+  %:comments
alias -l kte::RAW.252 %:echo $+(%::pre,,%::c1,%::value) $iif(%::value == 1,operator,operators) online
alias -l kte::Raw.408 %:echo 04[08:04]08error:  $+ %::chan $+  you cannot use colors on this channel %:comments
...
alias -l kte::RAW.265 %:echo 04[08:04]08Current local users: %::users max: %::value $+  %:comments
alias -l kte::RAW.253 %:echo 04[08:04]08 $+ %::value unknown connections %:comments
alias -l kte::RAW.001 %:echo 08Welcome %::text $+  %:comments
alias -l kte::RAW.482 %:echo 04[08:04]08error:  $+ %::chan $+  you aren't an op %:comments
alias -l kte::RAW.401 %:echo 04[08:04]08error:  $+ %::nick $+  no such nickname %:comments
alias -l kte::RAW.266 %:echo 04[08:04]08Current global users: %::users max: %::value $+  %:comments
alias -l kte::RAW.250 %:echo 04[08:04]08 $+ %::value total connections %:comments
alias -l kte::RAW.002 %:echo 08Your Host Is %::server $+ , running %::value $+  %:comments
alias -l kte::RAW.474 %:echo 04[08:04]08error:  $+ %::chan $+  banned from channel %:comments
alias -l kte::RAW.003 %:echo 08This server was created on %::value $+  %:comments
alias -l kte::Raw.251 %:echo 04[08:04]08There are %::users users online, %::text invisible, on %::value servers %:comments
alias -l kte::RAW.475 %:echo 04[08:04]08error:  $+ %::chan $+  key required %:comments
alias -l kte::RAW.324 %:echo 04[08:04]08 modes: 07[06 $+ %::modes $+ 07]08 %:comments
alias -l kte::RAW.403 %:echo 04[08:04]08error:  $+ %::chan $+  no such channel %:comments
alias -l kte::RAW.376 %:echo 04[08:04]08end of MOTD %:comments
alias -l kte::RAW.221 %:echo 04[08:04]08 usermode: %::modes $+  %:comments
alias -l kte::Raw.301 %:echo 04[08:04] %::nick is away: %::text $+  %:comments
alias -l kte::RAW.432 %:echo 04[08:04]08error:  $+ %::nick $+  invalid nickname %:comments
alias -l kte::RAW.404 %:echo 04[08:04]08error:  $+ %::chan $+  unable to send message %:comments
alias -l kte::RAW.353 %:echo 04[08:04]08names 07(06 $+ %::chan $+ 07):08 %::text $+  %:comments
alias -l kte::RAW.341 %:echo 04[08:04]08 $+ %::nick invited to %::chan $+  %:comments
alias -l kte::RAW.005 %:echo %::text $+  %:comments
alias -l kte::RAW.473 %:echo 04[08:04]08error:  $+ %::chan $+  invite only %:comments
alias -l kte::Raw.302 %:echo 04[08:04]08 userhost %::nick $+ : %::address $+  %:comments
alias -l kte::RAW.433 %:echo 04[08:04]08error:  $+ %::nick $+  is in use %:comments
alias -l kte::RAW.421 %:echo 04[08:04]08error:  $+ %::value $+  invalid command %:comments
alias -l kte::RAW.366 %:echo 04[08:04]08end of /names %::chan $+  %:comments
