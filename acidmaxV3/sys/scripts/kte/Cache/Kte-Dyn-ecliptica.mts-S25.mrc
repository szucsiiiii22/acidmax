...
if ($nick == $me) { %:echo 14[2Channel14] 1 Joining10 %::chan $+  %:comments | haltdef }
else { %:echo 14[2Join14] [10»14] 1[12 $+ %::nick $+ 1]1 [14 $+ %::address 1] %:comments | haltdef }
...
%:echo 14[2Part14] [10«14] 1[12 $+ %::nick $+ 1]1 [14 $+ %::address $+ 1] %:comments | haltdef
...
if (* iswm %::text) { set -nu1 %::parentext 1[12 $+ %::text $+ 1] }
  while (%i) {
    ...
    if (%tgt = 3) || (%tgt = 2) { %st = 1 }
if (!%tgt) || (%tgt = 3) || (%tgt = 1) { %:echo = echo $color(quit) -ti2 %::chan | %:echo 14[2Quit14] [10x14] %::nick [[ $+ %::address $+ ]] [ $+ %::text $+ ] %:comments }
    dec %i
  }
  unset %::chan
if ($query($nick)) { %:echo = echo $color(quit) -ti2 $nick | %:echo 14[2Quit14] [10x14] %::nick [[ $+ %::address $+ ]] [ $+ %::text $+ ] %:comments }
if (%st) { %:echo = echo $color(quit) -sti2 | %:echo 14[2Quit14] [10x14] %::nick [[ $+ %::address $+ ]] [ $+ %::text $+ ] %:comments }
  haltdef
...
if ($nick == $me) { %:echo = echo $color(nick) -sti2 | %:echo 14[2Nick14] Your new nick is [ $+ %::newnick $+ ) $+  %:comments }
  while (%i) {
    set -u1 %::chan $comchan($newnick, %i)
    %tgt = $gettok($hget(Kte_Events, %::chan), 7, 44)
if (%tgt != 2) || ($nick == $me) { %:echo = echo $color(nick) -ti2 %::chan | %:echo 14[2Nick14] %::nick 1changes nick to [ $+ %::newnick $+ ] %:comments }
    dec %i
  }
if ($query($newnick)) { unset %::chan | %:echo = echo $color(nick) -ti2 $newnick | %:echo 14[2Nick14] %::nick 1changes nick to [ $+ %::newnick $+ ] %:comments }
  haltdef
...
%:echo 14[2Mode14] [[ 1 $+ %::modes $+ 14] %:comments | haltdef
  unset %::modes
...
set -nu1 %::chan $chan
  ...
  set -nu1 %::modes $1-
%:echo 2Modes15:14 %::nick 2sets mode15:14 %::modes $+  %:comments | haltdef
  unset %::modes
unset %:echo %::nick %::address %::target %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick
}
...
set -nu1 %::chan $chan
if (* iswm %::text) { set -nu1 %::parentext 1[12 $+ %::text $+ 1] }
%:echo 14[2Topic14] 1[12 $+ %::nick $+ 1] sets new topic:10 $+ %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
on ^&*:KICK:#:{
  if ($chan !ischan) { return }
...
if (* iswm %::text) { set -nu1 %::parentext 1[12 $+ %::text $+ 1] }
  if ($knick == $me) {
%:echo 14[2Kick14] [10x14] You have been kicked by  $+ %::nick $+  from 14[2 $+ %::chan $+ 14] [[ $+ %::text $+ ]] $+  %:comments
    if (%tgt == 2) { set -u1 %:echo echo $color(kick) -ti2 $chan }
    else { set -u1 %:echo echo $color(kick) -seti2 }
%:echo 14[2Kick14] [10x14] You have been kicked by  $+ %::nick $+  from 14[2 $+ %::chan $+ 14] [[ $+ %::text $+ ]] $+  %:comments
    _left $chan
  }
else { %:echo 14[2Kick14]1 %::nick 12kick14 %::knick [[ $+ %::text $+ ]] - 14[2 $+ %::chan $+ 14] %:comments }
  haltdef
...
%:echo 14[2Invite14]1 %::nick 14has invited you to join 14[2 $+ %::chan $+ 14] %:comments | haltdef
...
set -nu1 %::nick $nick
if (* iswm %::text) { set -nu1 %::parentext 1[12 $+ %::text $+ 1] }
%:echo 14[2Error14] [1 $+ %::text $+ 14] %:comments | haltdef
...
set -nu1 %::address $address
if (* iswm %::text) { set -nu1 %::parentext 1[12 $+ %::text $+ 1] }
%:echo 14[6Notify14] 14[12 $+ %::nick $+ 12)1 is online [ 12 $+ %::address $+ 1] 14[ 1 $+ %::text $+ 14] %:comments | haltdef
unset %:echo %::nick %::address %::parentext %::me %::server %::port %::pre %::c? %::text
}
on ^&*:UNOTIFY:{
  set -u1 %:echo echo $color(notify) -ti2 $+ $iif(($_isactive) && (@* !iswm $active), a, s)
...
set -nu1 %::address $address
if (* iswm %::text) { set -nu1 %::parentext 1[12 $+ %::text $+ 1] }
%:echo 14[4Notify14] 14[12 $+ %::nick $+ 12]1 is Offline [12 $+ %::address $+ 1] 14[1 $+ %::text $+ 14] %:comments | haltdef
unset %:echo %::nick %::address %::parentext %::me %::server %::port %::pre %::c? %::text
}
...
%:echo 12DNS : 14Address %::iaddress ; %::naddress $+  %:comments | haltdef
      inc %i
    }
  }
  else {
    set -nu1 %::nick $nick
    set -nu1 %::address $address
%:echo 12DNS Error : 14unable to resolve %::iaddress ; %::naddress $+  %:comments | haltdef
  }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 1[12 $+ %::text $+ 1] }
%:echo 14[6Notice14] 14[12 $+ %::nick $+ 12) [[ 1 $+ %::text $+ 14] %:comments | haltdef
...
%:echo 14[ 12 $+ %::cmode $+ 5 $+ %::me $+ 14] 6 $+ %::text $+  %:comments
  }
  else {
    set -u1 %::cnick $cnick($me).color
%:echo 14[ 1 $+ %::me $+ 14]12 6 $+ %::text $+  %:comments
  }
...
else { %:echo 14[ 1 $+ %::me $+ 14]12 6 $+ %::text $+  %:comments }
  }
  else {
...
%:echo [2a1] 112 $+ %::cmode $+ %::nick $+ 1: 5 $+ %::text $+ 1 [2a1] $+  %:comments
...
else { %:echo 12 $+ %::cmode $+ %::mek $+ 1: 5 $+ %::text $+ 1 %:comments }
  }
...
if ($1 ischan) { set -u1 %::chan $1 | %:echo 14[6Notice14] [[ 1 $+ %::chan $+ 14] [1 $+ %::text $+ 14] %:comments }
else { %:echo 14[6Notice14] [[ 1 $+ %::text $+ 14] %:comments }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 1[12 $+ %::text $+ 1] }
%:echo 14[12 $+ %::cmode $+ 5 $+ %::nick $+ 14]6  $+ %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
on &^*:ACTION:*:#:{
  if ($chan !ischan) { return }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 1[12 $+ %::text $+ 1] }
%:echo [2a1] 112 $+ %::nick $+ 1: 5 $+ %::text $+ 1 [2a1] $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
on &^*:NOTICE:*:#:{
  if ($chan !ischan) { return }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 1[12 $+ %::text $+ 1] }
%:echo 14[6Notice14] [[ 1 $+ %::chan $+ 14] 14[12 $+ %::nick $+ 12) [1 $+ %::text $+ 14] %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
...
%:echo [2a1] 112 $+ %::nick $+ 1: 5 $+ %::text $+ 1 [2a1] $+  %:comments | haltdef
    ...
    set -nu1 %::text $strip($1-, o)
if (* iswm %::text) { set -nu1 %::parentext 1[12 $+ %::text $+ 1] }
%:echo 14[12 $+ %::nick $+ 14]1 6 $+ %::text $+  %:comments | haltdef
  }
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::text
}
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 1[12 $+ %::text $+ 1] }
if ($query($nick)) { set -u1 %:echo %e $nick | %:echo [2a1] 112 $+ %::nick $+ 1: 5 $+ %::text $+ 1 [2a1] $+  %:comments | haltdef }
  else {
    if ($window(Message Window)) { set -u1 %:echo %e $+ d }
    elseif ($_isactive) && (@* !iswm $active) && ($hget(Kte_Events, Active.Queries)) { set -u1 %:echo %e $+ a }
    else { set -u1 %:echo %e $+ s }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 1[12 $+ %::text $+ 1] }
%:echo 14[5Notice14] 14[12 $+ %::nick $+ 12) [1 $+ %::text $+ 14] %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::text
}
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 1[12 $+ %::text $+ 1] }
if (%ch) { %:echo 14[2Ctcp14] 14[12 $+ %::chan $+ 12]14[1 $+ %::ctcp $+ 14][1 $+ %::text $+ 14] %:comments }
else { %:echo 14[2Ctcp14] 1[12 $+ %::nick $+ 1]14 14[1 $+ %::ctcp $+ 14][1 $+ %::text $+ 14] %:comments }
...
if ($1 ischan) { set -u1 %::chan $1 | %:echo 14[2Ctcp14] 14[12 $+ %::chan $+ 12]14[1 $+ %::ctcp $+ 14][1 $+ %::text $+ 14] %:comments }
else { set -u1 %::nick $1 | %:echo 14[2Ctcp14] 1[12 $+ %::nick $+ 1]14 14[1 $+ %::ctcp $+ 14][1 $+ %::text $+ 14] %:comments }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 1[12 $+ %::text $+ 1] }
%:echo 14[2Ctcp Reply14] 14[12 $+ %::nick $+ 12]14[1 $+ %::ctcp $+ 14][1 $+ %::text $+ 14] %:comments | haltdef
...
set -nu1 %::c1 02 | set -nu1 %::c2 12 | set -nu1 %::c3 11 | set -nu1 %::c4 14
if (* iswm %::text) { set -nu1 %::parentext 1[12 $+ %::text $+ 1] }
%:echo 14[2Ctcp Reply14] 14[12 $+ %::chan $+ 12]14[1 $+ %::ctcp $+ 14][1 $+ %::text $+ 14] %:comments
...
alias -l kte::Error %:echo 12Error:1 %::text $+  %:comments
alias -l kte::RAW.311 w.hois %::nick %::address %::realname
alias -l kte::Raw.303 %:echo 14:14: %::text are online %:comments
alias -l kte::RAW.319 %:echo 12Channels14:14 %::chan $+  %:comments
alias -l kte::RAW.255 %:echo 2Clients14 %::users 14:: 12Servers14 %::value $+  %:comments
alias -l kte::RAW.471 %:echo [2&1] 14Can`t join15:14 [ $+ %::chan $+ ]1 is full %:comments
alias -l kte::Raw.332 %:echo 12Topic 14::14 %::text $+  %:comments
alias -l kte::RAW.252 %:echo 12Opers online 14::1 %::value $+  %:comments
alias -l kte::RAW.333 %:echo 14:14: 2Set by15:14 %::nick 12on %::text $+  %:comments
alias -l kte::RAW.317 idle.t %::idletime
alias -l kte::RAW.305 %:echo 12Away 14::1 You're no longer marked as being away %:comments
alias -l kte::RAW.265 %:echo 2Local users14 %::users 14:: 12Max14 %::value $+  %:comments
alias -l kte::RAW.253 %:echo 12Unknown Connections 14::1 %::value $+  %:comments
alias -l kte::Raw.001 %:echo 12Conected to 14::1 %::text $+  %:comments
alias -l kte::RAW.314 %:echo 2 $+ %::nick $+ 14 was 12[ $+ %::address $+ ]] 1[ $+ %::realname $+ ]] $+  %:comments
alias -l kte::RAW.306 %:echo 12Away 14::1 You're now marked as being away %:comments
alias -l kte::RAW.482 %:echo [2&1] 14You should be an IRC Operator %:comments
alias -l kte::RAW.401 %:echo [2&1] 14No such nickname15:14 [ $+ %::nick $+ ] %:comments
alias -l kte::RAW.266 %:echo 2Global users14 %::users 14:: 12Max14 %::value $+  %:comments
alias -l kte::RAW.250 %:echo 12Conectionsd 14::1 %::value $+  %:comments
alias -l kte::Raw.002 %:echo 12Host 14::1 %::address $+  %:comments
alias -l kte::RAW.474 %:echo [2&1] 14Can`t join15:14 [ $+ %::chan $+ ]1 Banned %:comments
alias -l kte::RAW.315 %:echo 12Who 14::14 end of /who 1[ $+ %::value $+ ]] $+  %:comments
alias -l kte::RAW.307 %:echo 12Reg Nick 14::14 %::nick $+  %:comments
alias -l kte::Raw.003 %:echo 12Created 14::1 %::value $+  %:comments
alias -l kte::Raw.251 %:echo 2 $+ %::users $+ 14 users 14:: 12 $+ %::text $+ 14 invisible 14:: 1 $+ %::value $+ 14 servers %:comments
alias -l kte::RAW.467 %:echo [2&1] 14Can`t join15:14 [ $+ %::chan $+ ]1 +k %:comments
alias -l kte::RAW.324 %:echo 12Channel Modes 14::14 %::modes $+  %:comments
alias -l kte::RAW.312 %:echo 12Server 14::14 %::wserver $+  %:comments
alias -l kte::RAW.403 %:echo [2&1] 14No such nickname15:14 [ $+ %::chan $+ ] %:comments
alias -l kte::RAW.313 %:echo 12Stat 14::4 IRC Operator %:comments
alias -l kte::RAW.221 %:echo 12UserModes 2 $+ %::nick 14::1 %::modes $+  %:comments
alias -l kte::Raw.301 %:echo 12Away 14::1 %::text $+  %:comments
alias -l kte::RAW.404 %:echo [2&1] 14Can`t send menssage to 15:14 [ $+ %::nick $+ ] %:comments
alias -l kte::RAW.353 %:echo 12Names 14::14 %::chan %::text $+  %:comments
alias -l kte::RAW.341 %:echo 12Invite 14::14 %::nick has been invited to 1 $+ %::chan $+  %:comments
alias -l kte::Raw.005 %:echo 12Protocols Supportedd 14::1 %::text $+  %:comments
alias -l kte::RAW.473 %:echo [2&1] 14Can`t join15:14 [ $+ %::chan $+ ]1 +i Invited only %:comments
alias -l kte::RAW.381 %:echo 12You are now a 14IRC Operator [[ $+ %::address $+ ]] $+  %:comments
alias -l kte::Raw.302 %:echo 12UserHost 14::1 %::nick [adress] [ $+ %::value $+ ) $+  %:comments
alias -l kte::RAW.433 %:echo [2&1] 14 $+ %::nick $+ 14 is alredy in use %:comments
alias -l kte::RAW.421 %:echo [2&1] 14Invalid Command15:14 [ $+ %::text $+ ]1 %:comments
alias -l kte::RAW.366 %:echo 12Names 14::14 end of /names %::chan $+  %:comments
alias -l kte::RAW.318
alias -l kte::RAW.254 %:echo 12Channels 14::1 %::value $+  %:comments
