...
; Kte-Dyn 1.5 FF4BFBB0 A90B06BE
alias theme.text {
  if (!$var(%:echo)) { return $false }
set -nu1 %::me $me | set -nu1 %::server $server | set -nu1 %::port $port
set -nu1 %::pre 12:11:12:
set -nu1 %::c1 11 | set -nu1 %::c2 12 | set -nu1 %::c3 02 | set -nu1 %::c4 11
...
if ($nick == $me) { %:echo 12:11:12:11 [12 $+ %::chan $+ 11] %:comments | haltdef }
else { krox.join | haltdef }
...
if ($nick($chan, $nick).pnick != $nick) { set -nu1 %::cmode $left($ifmatch, 1) } | set -nu1 %::cnick $nick($chan, $nick).color
set -nu1 %::chan $chan
if (* iswm %::text) { set -nu1 %::parentext 12[11 $+ %::text $+ 12] }
%:echo 12:11:12:11 part: %::nick 02[12 $+ %::address $+ 02] [12 $+ %::text $+ 02] %:comments | haltdef
...
if (* iswm %::text) { set -nu1 %::parentext 12[11 $+ %::text $+ 12] }
  while (%i) {
    ...
    if (%tgt = 3) || (%tgt = 2) { %st = 1 }
if (!%tgt) || (%tgt = 3) || (%tgt = 1) { %:echo = echo $color(quit) -ti2 %::chan | krox.quit }
    dec %i
  }
  unset %::chan
if ($query($nick)) { %:echo = echo $color(quit) -ti2 $nick | krox.quit }
if (%st) { %:echo = echo $color(quit) -sti2 | krox.quit }
  haltdef
...
if ($nick == $me) { %:echo = echo $color(nick) -sti2 | %:echo 12:11:12:11 nick: %::newnick $+  %:comments }
  while (%i) {
    set -u1 %::chan $comchan($newnick, %i)
    %tgt = $gettok($hget(Kte_Events, %::chan), 7, 44)
if (%tgt != 2) || ($nick == $me) { %:echo = echo $color(nick) -ti2 %::chan | %:echo 12:11:12:11 nick: %::nick 02[12 $+ %::address $+ 02]11 is now %::newnick $+  %:comments }
    dec %i
  }
if ($query($newnick)) { unset %::chan | %:echo = echo $color(nick) -ti2 $newnick | %:echo 12:11:12:11 nick: %::nick 02[12 $+ %::address $+ 02]11 is now %::newnick $+  %:comments }
  haltdef
...
%:echo 12:11:12:11 usermode: %::modes $+  %:comments | haltdef
  unset %::modes
...
set -nu1 %::chan $chan
  ...
  set -nu1 %::modes $1-
%:echo 12:11:12:11 mode: %::nick 02[12 $+ %::address $+ 02] 11set: %::modes $+  %:comments | haltdef
  unset %::modes
unset %:echo %::nick %::address %::target %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick
}
...
set -nu1 %::chan $chan
if (* iswm %::text) { set -nu1 %::parentext 12[11 $+ %::text $+ 12] }
%:echo 12:11:12:11 %::nick sets topic: 02[12 $+ %::text $+ 02] %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
on ^&*:KICK:#:{
  if ($chan !ischan) { return }
...
if (* iswm %::text) { set -nu1 %::parentext 12[11 $+ %::text $+ 12] }
  if ($knick == $me) {
%:echo 12:11:12:11 You were kicked from %::chan by %::nick 02[12 $+ %::text $+ 02] %:comments
    if (%tgt == 2) { set -u1 %:echo echo $color(kick) -ti2 $chan }
    else { set -u1 %:echo echo $color(kick) -seti2 }
%:echo 12:11:12:11 You were kicked from %::chan by %::nick 02[12 $+ %::text $+ 02] %:comments
    _left $chan
  }
else { %:echo 12:11:12:11 kick: %::nick 02[12 $+ %::address $+ 02]11 kicked %::knick 02[12 $+ %::text $+ 02] %:comments }
  haltdef
...
%:echo 12:11:12:11 invite: %::nick 02[12 $+ %::address $+ 02]11 invites you to join %::chan $+  %:comments | haltdef
...
set -nu1 %::nick $nick
if (* iswm %::text) { set -nu1 %::parentext 12[11 $+ %::text $+ 12] }
%:echo 12:11:12:11 error: %::text $+  %:comments | haltdef
...
set -nu1 %::address $address
if (* iswm %::text) { set -nu1 %::parentext 12[11 $+ %::text $+ 12] }
%:echo 12:11:12:11 nick: %::nick 02[12 $+ %::address $+ 02] is on irc %:comments | haltdef
unset %:echo %::nick %::address %::parentext %::me %::server %::port %::pre %::c? %::text
}
on ^&*:UNOTIFY:{
  set -u1 %:echo echo $color(notify) -ti2 $+ $iif(($_isactive) && (@* !iswm $active), a, s)
...
set -nu1 %::address $address
if (* iswm %::text) { set -nu1 %::parentext 12[11 $+ %::text $+ 12] }
%:echo 12:11:12:11 nick: %::nick 02[12 $+ %::address $+ 02] is off irc %:comments | haltdef
unset %:echo %::nick %::address %::parentext %::me %::server %::port %::pre %::c? %::text
}
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 12[11 $+ %::text $+ 12] }
%:echo 12!11 $+ %::nick $+ 12!11 %::text $+  %:comments | haltdef
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 12[11 $+ %::text $+ 12] }
%:echo 12-11 $+ %::server $+ 12-11 %::text $+  %:comments | haltdef
...
%:echo 12[ $+ %::cmode $+ 11 $+ %::nick $+ 12]11 %::text $+  %:comments
  }
  else {
    set -u1 %::cnick $cnick($me).color
%:echo 12[11 $+ %::nick $+ 12]11 %::text $+  %:comments
  }
...
else { %:echo 12[11 $+ %::nick $+ 12]11 %::text $+  %:comments }
  }
  else {
    set -u1 %:echo echo $color(own) -ti2 $+ $_active
%:echo 12-> *11 $+ %::target $+ 12*11 %::text $+  %:comments | haltdef
  }
...
else { %:echo 12* 11 $+ %::me %::text $+  %:comments }
  }
...
if ($1 ischan) { set -u1 %::chan $1 | %:echo 12-> -11 $+ %::target $+ 12-11 %::text $+  %:comments }
else { %:echo 12•11• notice sent 12» 11( $+ %::nick $+ )1211 %::text $+  %:comments }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 12[11 $+ %::text $+ 12] }
%:echo 12[ $+ %::cmode $+ 11 $+ %::nick $+ 12]11 %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
on &^*:ACTION:*:#:{
  if ($chan !ischan) { return }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 12[11 $+ %::text $+ 12] }
%:echo 12• 11 $+ %::nick %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
on &^*:NOTICE:*:#:{
  if ($chan !ischan) { return }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 12[11 $+ %::text $+ 12] }
%:echo 12-11 $+ %::nick $+ : $+ %::target $+ 12-11 %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::chan %::cmode %::cnick %::text
}
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 12[11 $+ %::text $+ 12] }
if ($query($nick)) { set -u1 %:echo %e $nick | %:echo 12* 11 $+ %::nick %::text $+  %:comments | haltdef }
  else {
    if ($window(Message Window)) { set -u1 %:echo %e $+ d }
    elseif ($_isactive) && (@* !iswm $active) && ($hget(Kte_Events, Active.Queries)) { set -u1 %:echo %e $+ a }
    else { set -u1 %:echo %e $+ s }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 12[11 $+ %::text $+ 12] }
%:echo 12-11 $+ %::nick 12[11 $+ %::address $+ 12]-11 %::text $+  %:comments | haltdef
unset %:echo %::nick %::address %::target %::parentext %::me %::server %::port %::pre %::c? %::text
}
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 12[11 $+ %::text $+ 12] }
if (%ch) { %:echo 12[11 $+ %::nick $+ : $+ %::chan %::ctcp $+ 12] 11 $+ %::text $+  %:comments }
else { %:echo 12[11 $+ %::nick %::ctcp $+ 12] 11 $+ %::text $+  %:comments }
...
if ($1 ischan) { set -u1 %::chan $1 | %:echo 12-> [11 $+ %::target %::ctcp $+ 12] 11 $+ %::text $+  %:comments }
else { set -u1 %::nick $1 | %:echo 12-> [11 $+ %::target %::ctcp $+ 12] 11 $+ %::text $+  %:comments }
...
set -nu1 %::target $target
if (* iswm %::text) { set -nu1 %::parentext 12[11 $+ %::text $+ 12] }
%:echo 12[11 $+ %::nick %::ctcp reply12] 11 $+ %::text $+  %:comments | haltdef
...
set -nu1 %::c1 11 | set -nu1 %::c2 12 | set -nu1 %::c3 02 | set -nu1 %::c4 11
if (* iswm %::text) { set -nu1 %::parentext 12[11 $+ %::text $+ 12] }
%:echo 12-> [11 $+ %::target %::ctcp reply12] 11 $+ %::text $+  %:comments
...
alias -l kte::Echo %:echo 11 $+ %::text $+  %:comments
alias -l kte::EchoTarget %:echo 11 $+ %::text $+  %:comments
alias -l kte::Error %:echo 12:11:12:11error: %::text $+  %:comments
alias -l kte::Raw.303 %:echo 12:11:12: %::text are online %:comments
alias -l kte::RAW.375 %:echo 12:11:12:11MOTD - %::server $+  %:comments
alias -l kte::RAW.255 %:echo 12:11:12:11I have %::users clients and %::value servers %:comments
alias -l kte::RAW.471 %:echo 12:11:12:11error:  $+ %::chan $+  is full %:comments
alias -l kte::RAW.391 %:echo 12:11:12:11server time: %::text $+  %:comments
alias -l kte::Raw.332 %:echo 02.::11 topic: 02[12 $+ %::text $+ 02]11 %:comments
alias -l kte::RAW.372 %:echo 11 $+ %::text $+  %:comments
alias -l kte::RAW.252 %:echo $+(%::pre,,%::c1,%::value) $iif(%::value == 1,operator,operators) online
alias -l kte::Raw.408 %:echo 12:11:12:11error:  $+ %::chan $+  you cannot use colors on this channel %:comments
alias -l kte::RAW.333 %:echo $+(,%::c3,.::) $+(,%::c1,set by:) $+(,%::c3,$chr(91),,%::c2,%::nick,,%::c3,$chr(93),,%::c1) on $+(,%::c3,$chr(91),,%::c2,$asctime($ctime(%::text),ddd mmm dd $+ $chr(91) yyyy h:nn tt),,%::c3,$chr(93),,%::c1)
alias -l kte::RAW.329 %:echo $+(,%::c3,.::) $+(,%::c1,channel created:) $+(,%::c3,$chr(91),,%::c2,$asctime(%::text,ddd mmm dd $+ $chr(91) yyyy h:nn tt),,%::c3,$chr(93))
alias -l kte::RAW.265 %:echo 12:11:12:11Current local users: %::users max: %::value $+  %:comments
alias -l kte::RAW.253 %:echo 12:11:12:11 $+ %::value unknown connections %:comments
alias -l kte::RAW.001 %:echo 11Welcome %::text $+  %:comments
alias -l kte::RAW.482 %:echo 12:11:12:11error:  $+ %::chan $+  you aren't an op %:comments
alias -l kte::RAW.401 %:echo 12:11:12:11error:  $+ %::nick $+  no such nickname %:comments
alias -l kte::RAW.266 %:echo 12:11:12:11Current global users: %::users max: %::value $+  %:comments
alias -l kte::RAW.250 %:echo 12:11:12:11 $+ %::value total connections %:comments
alias -l kte::RAW.002 %:echo 11Your Host Is %::server $+ , running %::value $+  %:comments
alias -l kte::RAW.474 %:echo 12:11:12:11error:  $+ %::chan $+  banned from channel %:comments
alias -l kte::RAW.003 %:echo 11This server was created on %::value $+  %:comments
alias -l kte::Raw.251 %:echo 12:11:12:11There are %::users users online, %::text invisible, on %::value servers %:comments
alias -l kte::RAW.475 %:echo 12:11:12:11error:  $+ %::chan $+  key required %:comments
alias -l kte::RAW.324 %:echo 02.::11 modes: 02[12 $+ %::modes $+ 02]11 %:comments
alias -l kte::RAW.403 %:echo 12:11:12:11error:  $+ %::chan $+  no such channel %:comments
alias -l kte::RAW.376 %:echo 12:11:12:11end of MOTD %:comments
alias -l kte::RAW.221 %:echo 12:11:12:11 usermode: %::modes $+  %:comments
alias -l kte::Raw.301 %:echo 12:11:12: %::nick is away: %::text $+  %:comments
alias -l kte::RAW.432 %:echo 12:11:12:11error:  $+ %::nick $+  invalid nickname %:comments
alias -l kte::RAW.404 %:echo 12:11:12:11error:  $+ %::chan $+  unable to send message %:comments
alias -l kte::RAW.353 %:echo 02.::11 names 02[12 $+ %::chan $+ 02]:11 %::text $+  %:comments
alias -l kte::RAW.341 %:echo 12:11:12:11 $+ %::nick invited to %::chan $+  %:comments
alias -l kte::RAW.005 %:echo %::text $+  %:comments
alias -l kte::RAW.473 %:echo 12:11:12:11error:  $+ %::chan $+  invite only %:comments
alias -l kte::Raw.302 %:echo 12:11:12:11 userhost %::nick $+ : %::address $+  %:comments
alias -l kte::RAW.433 %:echo 12:11:12:11error:  $+ %::nick $+  is in use %:comments
alias -l kte::RAW.421 %:echo 12:11:12:11error:  $+ %::value $+  invalid command %:comments
alias -l kte::RAW.366 %:echo 02.::11 end of /names %::chan $+  %:comments
