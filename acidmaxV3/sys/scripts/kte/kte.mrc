...
alias kte_load {
  ...
  if (!$0) { kte_dlg -m kte_load | return }
...
dialog -l kte_load {
  ...
  list 2, 3 12 59 80, sort size vsbar
  text "Apply &scheme:", 20, 3 93 35 7, disable
  list 21, 3 101 59 39, disable size
  ...
  text "Double-click here to generate preview", 30, 97 78 91 7, tab 1001 center
  ...
  box "", 2900, -2 140 225 60
  button "Load", 2901, 136 147 40 11, disable default ok
  button "Close", 2902, 179 147 40 11, cancel
  menu "&File", 43
...
on *:DIALOG:kte_load:sclick:2901:{
  ...
  if ($dialog(Switch)) { .timer -m 1 100 switch.on }
  if (%statusbar == 1) { .timer -m 1 50 status.off | .timer -m 1 150 status.on }
}
