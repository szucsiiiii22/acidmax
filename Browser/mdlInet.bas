...
DoEvents
SetUpdateBusy True
If FTPConnect("Team Nexgen", "ftp.tnexgen.com", "21", "acidmax2updates@tnexgen.com", "f3rh431j76#", True, True) = True Then
    LoginToFTP = True
Else
    LoginToFTP = False
End If
...
If session <> 0 Then
    ...
    End If
Else
    AddToFileList "Connection to server failed." & vbCrLf
    InternetCloseHandle session
    Exit Function
End If
End Function
...
If session = 0 Or server = 0 Then
    AddToFileList "You are not connected to any server"
    Exit Sub
End If
DoEvents
adr = Replace(adr, "*.**.*", "*.*")
...
If Len(lFileName) <> 0 Then
    ...
    If f <> 0 Then
        If ReturnSilent = True And LCase(lFileName) <> "version.ini" Then mIRCSend "//echo Downloading: " & lFileName
        z = ReturnFileSize(f)
        spRate = 0
        sAllBytes = 0
        p = 0
