...
Option Explicit
Private ID As String, Pass As String, port As Integer, session As Long, server As Long, Transfer As Long, Adresa As String, strPath As String, adr As String, Klic As String
...
Private Type FtpDirectoryInformation
    FtpFile As String
    FtpModified As String
    FtpSize As String
    FtpRemotePath As String
End Type
Private FtpDirectories() As FtpDirectoryInformation
Private MaxFtpDirectory As Integer
...
If FTPConnect("Team Nexgen", "team-nexgen.org", "21", "websites", "tellarite#1", True, True) = True Then
    LoginToFTP = True
Else
    LoginToFTP = False
End If
...
Private Function FTPConnect(ftpSiteName As String, ftpServer As String, ftpPort As String, ftpUserId As String, ftpPassword As String, ftpBinary As Boolean, ftpPassive As Boolean) As Boolean
On Local Error GoTo ErrHandler
Dim Service As Long, l As Long, o As Long
If Len(ftpServer) <= 6 Then
    Exit Function
End If
Adresa = ftpServer
ID = ftpUserId
...
Klic = ""
If ftpSiteName = "" Then
    ftpSiteName = App.Title
End If
If ftpBinary = True Then
    Transfer = FTP_TRANSFER_TYPE_BINARY
ElseIf ftpBinary = False Then
    Transfer = FTP_TRANSFER_TYPE_ASCII
End If
If ftpPassive = True Then
    Service = INTERNET_FLAG_PASSIVE
ElseIf ftpPassive = False Then
...
session = CallFuncPtr(l, ftpSiteName, INTERNET_OPEN_TYPE_DIRECT, "", "", INTERNET_FLAG_NO_CACHE_WRITE)
If session <> 0 Then
...
Public Function ListFTPDirectory() As Collection
On Local Error GoTo ErrHandler
Dim hFile As Long, udtWFD As WIN32_FIND_DATA, strFile As String, Img As Integer, r As Integer, l&, sTime As SYSTEMTIME, lTime As FILETIME, lExit As Boolean, msg As String ', result As New Collection
...
If hFile Then
    Do
        ...
        If Len(strFile) > 0 Then
            Dim dir As FtpDirectoryInformation
            If udtWFD.dwFileAttributes And vbDirectory Then
                dir.FtpFile = Klic & strFile
                dir.FtpModified = ""
                ...
                If Len(Trim(strFile)) <> 0 Then
                    If DoesFileExistInType(strFile) = False Then
                        ...
                        If ReturnFinishedCheckingFiles = False Then
                            ...
                            Case Else
                                dir.FtpFile = strFile
                                dir.FtpModified = Trim(CalcFTime(sTime))
                                dir.FtpSize = Str(udtWFD.nFileSizeLow)
                                dir.FtpRemotePath = adr
                                FtpDirectories.Add dir
                            End Select
                        End If
                    End If
