...
On Local Error GoTo ErrHandler
SetMainFormStatusLabel "Connecting to FTP server"
AddToStatus "Connecting to Server"
DoEvents
SetFTPBusy True
If FTPConnect("Team Nexgen", "ftp.tnexgen.com", "21", "acidmax2betaupdates@tnexgen.com", "734j89ew1", True, True) = True Then
    LoginToFTP = True
Else
    MsgBox "Unable to connect to FTP server", vbExclamation
    LoginToFTP = False
    SetFTPBusy False
End If
Exit Function
...
port = lPort
Klic = ""
If lSiteName = "" Then lSiteName = App.Title
...
End If
session = InternetOpen(lSiteName, INTERNET_OPEN_TYPE_DIRECT, "", "", INTERNET_FLAG_NO_CACHE_WRITE)
If session <> 0 Then
...
Public Sub ListFTPDirectory(Optional lDirectory As String)
On Local Error Resume Next
...
On Local Error GoTo ErrHandler
Dim sBuffer As String, FileData As String, Ret As Long, SentBytes As Long, sAllBytes As Long, z As Long, i As Integer, FF As Integer, Kam As String, Ode As String, Fs As Long, StartT As Long, t As Long, Cnt As Long, p As Long, spRate As Single, hFile As Long, f As Integer
...
If Len(lFileName) <> 0 Then
    ...
    If f <> 0 Then
        ...
        'If DoesListBoxItemExist(lFileName, ReturnMainFormFilesListBox) = True Then
        '    RemoveMainFormFilesListBoxItem lFileName
        'End If
    End If
End If
If LCase(lFileName) <> "version.ini" Then
